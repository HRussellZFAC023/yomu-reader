...
- Local Study stays available without an account. Server profiles, pairing, and sync require a Google account; the HMAC-matched `UCL2026` session is the explicit zero-friction anonymous exception. The Class Board still requires an account and explicit visibility opt-in.
...
- Stripe Checkout first creates a pending purchase. A paid code remains retryable and binds atomically only after verified Google sign-in, with one code per account and one account per code. Account deletion retains only a non-identifying redemption tombstone so codes cannot become transferable.
