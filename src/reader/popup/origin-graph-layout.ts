...
import type { OriginGraphNode, OriginGraphEdge, OriginEdgeGroup, PositionedOriginNode } from './origin-graph';
...
export function isOriginSubcomponentEdge(edge: OriginEdgeGroup): boolean {
    return edge.labels.includes('subcomponent');
}

export function groupOriginEdges(edges: OriginGraphEdge[]): OriginEdgeGroup[] {
...
export function forceLayoutOriginGraph(nodes: OriginGraphNode[], edges: OriginEdgeGroup[], currentId: string): PositionedOriginNode[] {
...
export function inferInboundComponentZone(node: OriginGraphNode, currentId = ''): OriginComponentZone {
    const override = inboundPlacementOverride(currentId, node);
...
export function inferOutboundComponentZone(currentId: string, node: OriginGraphNode): OriginComponentZone {
...
export function clippedOriginEdgePath(from: PositionedOriginNode, to: PositionedOriginNode, targetZone: GraphAnchorZone): ReturnType<typeof graphEdgePath> {
...
export function hashOriginGraphId(value: string): string {
    let hash = 0;
