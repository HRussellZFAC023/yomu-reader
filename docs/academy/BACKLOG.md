...
**Active task:** `day:1:lesson-zero:09` - turn the two-name listening probe into a short, natural classroom exchange: teach the exact names before testing them, require recognition from accepted character audio rather than written guessing, provide one bounded repair, change speaker/order for transfer, persist the exact cursor, seed only stable SRS identities, and prove story reachability, pause/reload, responsive accessibility and the return seam on phone, tablet and desktop.
