import { legacyDocsMessageId, type MessageId } from "../../../src/reader/locales/message-ids";

export type ReviewedDocsLocale = "en" | "ja";

export interface ReviewedDocsMessage {
...
const HOSTED_DOCS_JA_COPY: Record<string, string> = {
    ...
    'Theme and interface-language previews now stay separate from the last saved settings, so Save records the new explicit choice instead of letting an older choice return after reload. A settings dialog reopened after backup restore also keeps ownership, so Save visibly completes.': 'テーマとインターフェース言語のプレビューを、最後に保存した設定から分離しました。保存時に新しい明示的な選択が記録され、再読み込み後に古い選択が戻ることはありません。バックアップ復元後に開き直した設定画面でも、保存完了が正しく表示されます。',
    'If Study and ordinary websites already show different chosen settings, update first, then import your latest settings backup once from Study. Do not factory reset or downgrade.': 'Studyと通常のウェブサイトで既に異なる選択済み設定が表示される場合は、まず更新してから、最新の設定バックアップをStudyで一度インポートしてください。ファクトリーリセットやダウングレードは行わないでください。',
    "Yomu Gaming's Reader settings shortcut now opens the native Settings window instead of an inert Study launcher, and its save confirmation follows the selected interface language.": 'Yomu GamingのReader設定ショートカットは、操作できないStudyランチャーではなく、ネイティブの設定ウィンドウを開くようになりました。保存完了の表示も、選択中のインターフェース言語に従います。',
    'Subtitle rail icons respond reliably when a mouse, pen, or finger lands on their nested SVG artwork, without turning a drag into a click or keeping YouTube\'s native controls awake. The off-site Study settings launcher also stays fully visible on iPhone and iPad WebKit.': '字幕レールのアイコン内にあるSVG要素をマウス、ペン、指で直接操作しても確実に反応し、ドラッグがクリックとして扱われたり、YouTube本来のコントロールが表示されたままになったりしなくなりました。サイト外に表示するStudy設定ランチャーも、iPhoneとiPadのWebKitで画面内に収まるようになりました。',
    'Restored YouTube subtitles for existing learners after the 1.9.0 learning-target migration. Pre-1.9 records that contain actual Reader or subtitle settings keep their historical target; passive hosted appearance and bootstrap records stay unchosen. An open YouTube tab now starts its subtitle reader when a target is chosen in another tab.': '1.9.0の学習対象移行後、既存の学習者向けYouTube字幕を復旧しました。実際のReaderまたは字幕設定を含む1.9より前の保存データは従来の学習対象を維持しますが、ホスト版の外観と初期化だけのデータは未選択のままです。別のタブで学習対象を選ぶと、開いたままのYouTubeタブでも字幕Readerが起動します。',
    'Fresh installs remain language-neutral: Academy and hosted appearance preferences now stamp their neutral target state, and a rejected onboarding save cannot become active after reload.': '新規インストールは言語未選択の状態を維持します。Academyとホスト版の外観設定は学習対象が未選択であることを明示的に記録し、初期設定の保存に失敗した場合も、再読み込み後に有効になることはありません。',
    'Reader controls are now isolated from host-page scripts: arbitrary sites cannot trigger privileged settings, subtitle, card, or OCR actions; rewrite values behind a real click; or read credentials, account details, imported files, recovery codes, and captured raster data. Sensitive setup opens on Yomu-owned Study and extension new-tab surfaces while ordinary reading stays on the page.': 'Readerの操作をホストページのスクリプトから分離しました。任意のサイトから、特権を伴う設定・字幕・カード・OCR操作を起動したり、実際のクリック直前に値を書き換えたり、認証情報・アカウント詳細・読み込んだファイル・復旧コード・取得した画像データを読み取ったりすることはできません。機密性の高いセットアップは、よむが所有するStudyと拡張機能の新規タブ画面で開き、通常の読解は元のページ上で使えます。',
    "Yomu Gaming's packaged overlay again annotates captured text after the private-storage hardening, and Steam Deck/gamepad navigation finds those words without exposing card identifiers in the page DOM.": '非公開ストレージの強化後も、Yomu Gamingのパッケージ版オーバーレイがキャプチャーした文字へ再び注釈を付けるようになりました。Steam Deckとゲームパッドのナビゲーションは、ページのDOMにカード識別子を公開せずに、その単語を見つけます。',
    'Learning target selected': '学習対象を選択済み',
    'Records whether you chose a learning target. Until you do, target-specific reading, dictionary, OCR, and Study work stays off.': '学習対象を選んだかどうかを記録します。選ぶまでは、対象言語に固有の読解、辞書、OCR、Studyの処理は動きません。',
    'Uchisen is not an embedded kanji source. It remains available only as a disabled-by-default outbound lookup link; よむ does not fetch or render its pages, mnemonic stories, images, keywords, or components. Older saved `uchisenEnabled`, `uchisenAlias`, and `uchisenPriority` fields are retained for settings compatibility but ignored.': 'Uchisenは組み込みの漢字ソースではありません。既定で無効な外部検索リンクとしてのみ残り、よむがページ、記憶用ストーリー、画像、キーワード、構成要素を取得・表示することはありません。以前に保存された`uchisenEnabled`、`uchisenAlias`、`uchisenPriority`は設定互換性のため保持しますが、使用しません。',
    'The extension asks to run on websites because its core purpose is to add reading, target-aware lookup, OCR, subtitle, and mining tools for your selected learning language to the page you are viewing. It uses `activeTab` for user-requested visible-tab capture, `scripting` to install the reader, `storage` for settings and local study data, and context-menu access for reader shortcuts. It does not request browsing-history access.': '拡張機能がウェブサイトでの実行権限を求めるのは、閲覧中のページに、選んだ学習言語向けの読解、対象言語に合う検索、OCR、字幕、単語保存の機能を追加することが主な目的だからです。`activeTab`は利用者が指示した表示中タブの取得、`scripting`はリーダーの導入、`storage`は設定と端末内の学習データ、コンテキストメニュー権限はリーダーのショートカットに使います。閲覧履歴へのアクセス権限は求めません。',
    'The same owned-page boundary protects authoritative settings and onboarding choices, imports, pairing and recovery controls, account-backed study details, and captured OCR image data. On an ordinary website, Yomu keeps those values out of page-readable controls and opens Study instead; popup lookup, annotations, and subtitles still work on the page you are reading.': '同じ所有ページの境界によって、正式な設定と初期設定の選択、インポート、ペアリングと復旧の操作、アカウントに基づく学習情報、OCRで取得した画像データも保護します。通常のウェブサイトでは、よむはそれらの値をページから読めるコントロールに置かず、代わりにStudyを開きます。ポップアップ検索、注釈、字幕は読んでいるページ上で引き続き使えます。',
    "Study's Translation card now follows the selected learning target instead of silently disappearing outside Japanese. Spanish, Arabic, Korean, and the other provider-supported targets send the correct source language, keep source and translated text marked with their own language and writing direction, and leave Ancient Greek explicitly unavailable rather than inventing a result.": 'StudyのTranslationカードは、日本語以外で黙って消えず、選択中の学習対象に従うようになりました。スペイン語、アラビア語、韓国語などプロバイダーが対応する対象では正しい原文言語を送り、原文と翻訳結果にそれぞれの言語と書字方向を設定します。古代ギリシャ語は結果を捏造せず、明示的に利用不可と表示します。',
    'Canonically equivalent text no longer becomes a fake reading annotation. Decomposed text such as Spanish año and its NFC dictionary spelling are treated as the same surface, while Japanese furigana and exact parser-owned lookup spans remain unchanged.': 'Unicode上で正規化すると同じテキストに、偽の読み注釈を付けなくなりました。スペイン語の分解形「año」とNFC形式の辞書表記は同じ表層として扱い、日本語のふりがなとパーサーが所有する正確な検索範囲は変更しません。',
    "New Tab's My Cards browser now stays on the selected learning target before provider limits and deduplication. Legacy JPDB, Jiten, and Anki cards remain Japanese instead of being relabelled as Spanish or another active target; rows use their own language, writing direction, and collation defensively.": 'New Tabの「My Cards」は、プロバイダーの件数制限や重複排除より前に、選択中の学習対象だけへ絞り込むようになりました。従来のJPDB・Jiten・Ankiカードは、スペイン語など現在の対象として付け替えず、日本語のまま扱います。各行は防御的にカード自身の言語、書字方向、照合規則を使用します。',
    'The hosted homepage Try me reader now opens lookups consistently on both English and Japanese routes under the same page-owned language policy used in production. Release preview coverage verifies that behavior without granting the pseudo-loopback preview host broad userscript-storage access.': 'ホスト版ホームページの「試してみる」Readerは、本番と同じページ所有の言語ポリシーに従い、英語ルートと日本語ルートの両方で一貫して検索を開くようになりました。リリース用プレビューでは、疑似ループバックのプレビューホストに広範なユーザースクリプト保存領域へのアクセスを与えずに、この動作を検証します。',
    'A fresh installation now waits for the learner to choose a learning target instead of silently activating Japanese. Dismissing the chooser keeps the Reader inert and asks again later; completed and non-Japanese profiles from earlier releases remain intact.': '新規インストールでは、日本語を自動的に有効にせず、学習者が学習対象を選ぶまで待つようになりました。選択画面を閉じるとReaderは何もせず、後でもう一度確認します。以前のリリースで完了済みの設定や日本語以外のプロファイルはそのまま保持されます。',
    'Yomu Gaming now requires a fresh install to choose the capture language before any screen frame or OCR request can start. The tray, global shortcut, settings window, and overlay all fail closed until that explicit choice, while existing chosen targets remain intact.': 'Yomu Gamingの新規インストールでは、画面フレームの取得やOCR要求を始める前に、キャプチャー言語の選択が必要になりました。明示的に選ぶまでは、トレイ、グローバルショートカット、設定ウィンドウ、オーバーレイのすべてが安全側で停止します。既に選択済みの学習対象はそのまま保持されます。',
    'The dictionary catalogue opens on demand instead of eagerly mounting thousands of controls. Its collapsed form renders no dictionary cards, expansion preserves search, focus, update state, and English, Japanese, and right-to-left layout, and only current entries with an installable archive are listed; source-only Guide rows and legacy JMdict builds are no longer offered as if they could be installed.': '辞書カタログは数千個の操作部を先に生成せず、必要なときだけ開くようになりました。折りたたみ時は辞書カードを描画せず、展開しても検索、フォーカス、更新状態、英語・日本語・右から左のレイアウトを維持します。インストール可能なアーカイブを持つ現行項目だけを一覧にし、ソースのみのGuide行や旧JMdictビルドを、インストールできるかのようには表示しません。',
    'Release browser evidence now checks exact glyph, span, popup, language, and writing-direction ownership in Chromium and Firefox for Japanese plus representative Spanish, Arabic, Korean, and Cantonese text, with a roster contract covering all 33 learning targets.': 'リリース時のブラウザー証拠は、日本語に加えて代表的なスペイン語、アラビア語、韓国語、広東語について、正確な文字、範囲、ポップアップ、言語、書字方向の所有権をChromiumとFirefoxで確認するようになりました。33の学習対象すべてを網羅する名簿契約も検証します。',
    'Switching learning targets now reconciles every target-owned surface together: page annotations, popup lookup providers, subtitles, OCR, Study, onboarding, and the floating puck no longer retain Japanese or a previous language\'s actions, labels, tracks, or provider URLs.': '学習対象を切り替えると、ページ注釈、ポップアップの検索先、字幕、OCR、Study、初期設定、フローティングパックなど、対象言語に属するすべての画面がまとめて更新されます。日本語や以前の言語の操作、ラベル、トラック、検索先URLが残ることはありません。',
    'OCR fallback words are segmented with the active learning target instead of the Japanese segmenter, so recognised Spanish and other supported text remains hoverable even when a parser returns no tokens. Text from a different target is reported as absent rather than appearing ready but non-interactive.': 'OCRのフォールバック単語は、日本語用の分割器ではなく現在の学習対象に合わせて分割されます。パーサーがトークンを返さない場合でも、認識されたスペイン語などの対応テキストをホバーできます。別の対象言語のテキストは、操作できないのに準備完了と表示せず、見つからなかったものとして扱います。',
    'YouTube subtitle discovery follows the active target and definition/translation language, invalidates stale generated tracks after a target switch, and keeps language and writing direction correct across the overlay, transcript, fullscreen mirror, and mined text.': 'YouTubeの字幕検出は、現在の学習対象と定義・翻訳言語に従います。対象を切り替えた後は古い生成トラックを無効にし、オーバーレイ、文字起こし、フルスクリーンのミラー、保存テキストの言語と書字方向を正しく保ちます。',
    'Closing the subtitle transcript after resuming annotations now rescans the page content it had covered, so YouTube titles and sidebar words return immediately instead of remaining plain.': '注釈を再開してから字幕トランスクリプトを閉じると、パネルに隠れていたページ内容を再スキャンします。YouTubeのタイトルやサイドバーの単語がプレーンなまま残らず、すぐに注釈表示へ戻ります。',
    'Subtitle discovery is offered only when the page contains a video the subtitle controller can actually use. YouTube feed previews no longer expose an action that cannot activate.': '字幕検出は、字幕コントローラーが実際に利用できる動画がページにある場合にだけ表示されます。YouTubeフィードのプレビューには、実行できない操作を表示しません。',
    'Japanese-site navigation is available only for the Japanese target and never follows production language links on loopback development origins, including IPv6, trailing-dot localhost, and IPv4-mapped loopback addresses.': '日本語版サイトへの移動は、日本語を学習対象にしている場合だけ利用できます。IPv6、末尾にドットがあるlocalhost、IPv4射影ループバックアドレスを含む開発用のローカルオリジンでは、本番環境向けの言語リンクをたどりません。',
    'English and other non-Japanese targets no longer inherit Japanese-only lookup pills, the Japanese site-language action, kana/kanji wording, or a Japanese OCR glyph. Built-in providers are regenerated for the current target while custom links and their enabled state are preserved.': '英語など日本語以外の学習対象に、日本語専用の検索ピル、日本語版サイトの操作、かな・漢字向けの文言、日本語OCRの字アイコンが引き継がれることはありません。組み込み検索先は現在の対象に合わせて作り直し、カスタムリンクと有効・無効の状態は維持します。',
    'Subtitle text is published only when that exact cue\'s annotation frame is settled, so words no longer gain furigana, pitch, or study colouring after appearing. Native captions retain ownership while a cue is pending or fails, and are restored if よむ is replaced or shut down.': '字幕テキストは、そのキューに対応する注釈フレームの準備が完了してから表示されます。表示後にふりがな、ピッチ、学習状態の色が追加されることはありません。キューが処理中または失敗した場合はネイティブ字幕が引き続き表示を担当し、よむの再起動・終了時にも元に戻ります。',
    'Switching subtitle tracks cancels stale caption, translation, and enrichment work before it can block or overwrite the new selection. Rate-limited and empty YouTube caption responses use bounded, source-aware retries instead of repeatedly walking every format and translation fallback.': '字幕トラックを切り替えると、古い字幕、翻訳、補足処理を取り消し、新しい選択の開始を妨げたり結果を上書きしたりしないようにします。レート制限または空のYouTube字幕応答は、すべての形式と翻訳フォールバックを繰り返すのではなく、ソースを識別した回数制限付きの再試行で処理します。',
    'Target-specific reading settings survive temporary target changes and live settings updates. A Japanese difficult-kanji preference is restored on return to Japanese, unsupported modes cannot leak into another target, and unrelated cross-tab changes no longer overwrite unsaved language or provider edits.': '対象言語別の読み注釈設定は、一時的な対象切り替えやライブ設定更新を経ても維持されます。日本語の「難しい漢字だけ」は日本語へ戻ると復元され、非対応モードが別の対象に残らず、別タブの無関係な変更が未保存の言語・検索先編集を上書きしません。',
    'Website locale changes load the reviewed server-rendered document atomically, so the previous language cannot paint at the new locale URL. Document language, direction, accessible navigation labels, canonical URL, Open Graph metadata, hreflang links, and structured breadcrumbs stay aligned; routes without reviewed Japanese copy fall back to the Japanese homepage instead of linking to a translated 404.': 'ウェブサイトの表示言語を切り替えると、確認済みのサーバー描画文書が一度に読み込まれるため、新しい言語のURLで以前の言語が表示されることはありません。文書の言語と方向、アクセシブルなナビゲーション名、正規URL、Open Graphメタデータ、hreflangリンク、構造化パンくずが整合します。確認済みの日本語版がないページは、翻訳版の404へリンクせず日本語ホームへ戻ります。',
    "The public homepage demo and Academy load the final userscript's SRI-pinned dependency graph before the Reader core. Both surfaces annotate normally again after the aggregate-runtime split, and a missing or partially deployed dependency now stops the core instead of producing a broken Reader.": '公開ホームページのデモとAcademyは、最終ユーザースクリプトのSRIで固定された依存関係グラフをReader本体より先に読み込むようになりました。集約ランタイムの分割後も両画面で注釈が再び正常に動作し、依存ファイルが欠けている場合やデプロイが途中の場合は、壊れたReaderを起動せず本体を停止します。',
    'The public homepage Try me sentence now stays beneath a stationary pointer while its rotating language headline changes. Each pre-rendered word carries exact source geometry for parser-authoritative hover lookup, so the card opens for the word the visitor chose instead of whichever unrelated annotation happens to move under the pointer.': '公開ホームページの「Try me」文は、言語見出しが切り替わっても静止したポインターの下に留まるようになりました。事前描画された各単語には、パーサーが主導するホバー検索のための正確な元文位置を持たせ、カーソル下へ偶然移動した無関係な注釈ではなく、訪問者が選んだ単語のカードを開きます。',
    'Yomu Gaming settings actions stay compact in desktop windows. Add-source, copy-address, account, help, and Academy buttons wrap naturally without stretching into giant full-width or full-height controls; narrow windows retain touch-friendly full-width actions.': 'Yomu Gamingの設定操作は、デスクトップウィンドウでコンパクトに収まるようになりました。ソース追加、アドレスコピー、アカウント、ヘルプ、Academyの各ボタンは、巨大な全幅・全高の操作部へ引き伸ばされず自然に折り返します。狭いウィンドウでは、タッチしやすい全幅の操作を維持します。',
    "The Reader built into yomureader.com now keeps the website's chosen language instead of redirecting a fresh English visit to Japanese. Opening Japanese versions of sites is opt-in for fresh installs; existing stored choices remain unchanged. Homepage, Study, PDF, Video, and Academy fallbacks keep that page-local policy separate, while an installed userscript or extension retains its own shared language preference and settings. Academy also re-announces reading surfaces when a late Reader becomes ready, so Firefox no longer sometimes leaves its Japanese copy plain.": 'yomureader.com内蔵のReaderは、新規の英語ページ訪問を日本語へ転送せず、ウェブサイトで選んだ表示言語を維持するようになりました。新規インストールでは、日本語版サイトを開く機能は明示的に有効にした場合だけ動作し、既存の保存済み設定は変更しません。ホームページ、Study、PDF、Video、Academyのフォールバックは、このページ固有の方針を分離して扱い、インストール済みのユーザースクリプトまたは拡張機能は、独自の共有言語設定とその他の設定を保持します。Academyでは、遅れてReaderの準備が整ったときに読み注釈の対象を再通知するため、Firefoxで日本語の文章が時折プレーン表示のまま残ることもありません。',
    'All 33 learning targets now have executable grammar, lookup, annotation, subtitle, OCR, mining, grading, and Study capability evidence. The audit distinguishes data-backed, target-adapted, generic-fallback, and unavailable behavior instead of declaring shallow boolean parity; morphology remains explicitly unavailable for targets without a real adapter.': '33の学習対象すべてに、文法、検索、注釈、字幕、OCR、保存、採点、Studyの実行可能な機能証拠を用意しました。監査は単純な真偽値で同等と宣言せず、データに基づく機能、対象別の実装、汎用フォールバック、利用不可を区別します。実際のアダプターがない対象では、形態解析を明示的に利用不可としています。',
    'The public site now uses static, hydration-safe English and Japanese locale routes with server-rendered copy and locale-specific navigation, metadata, search, and accessibility text. Seventeen Japanese routes are published from reviewed copy; four Japanese routes and the other 31 website locales remain unavailable until their prose and RTL requirements are reviewed.': '公開サイトは、サーバーで描画した文章と、言語別のナビゲーション、メタデータ、検索、アクセシビリティ文言を備えた、ハイドレーション時にも安全な英語・日本語の静的ルートを使うようになりました。確認済みの文章から日本語17ルートを公開しています。日本語4ルートと他の31言語は、本文とRTL要件の確認が終わるまで利用できません。',
    'Touch popup hydration coalesces late provider results into one post-paint render per card state. In the fixed YouTube mobile profile, popup scroll-state capture fell from 202.5 ms across 349 samples to 5.0 ms across 6 samples, while all four lookups opened and Escape removal completed in 25.6–36.5 ms.': 'タッチ操作のポップアップでは、遅れて届く検索先の結果を、カード状態ごとに描画後の1回の更新へまとめます。修正後のYouTubeモバイル計測では、ポップアップのスクロール状態取得が349サンプル・202.5ミリ秒から6サンプル・5.0ミリ秒に減り、4回すべての検索が開き、Escapeによる削除は25.6〜36.5ミリ秒で完了しました。',
    'The YouTube profiler now separates normal timing, CPU sampling, and precise call coverage, waits for the exact painted target occurrence, records popup mount and dismissal in page time, and fails with structured diagnostics when a replay cannot be compared.': 'YouTubeプロファイラーは、通常の時間計測、CPUサンプリング、正確な呼び出しカバレッジを分離し、実際に描画された対象の正しい出現位置を待つようになりました。ポップアップの表示と終了をページ時間で記録し、再生結果を比較できない場合は構造化された診断情報とともに失敗します。',
    ...
    '. Choose Add, select Japanese as your learning target in Yomu, then open a Japanese page.': 'を使います。追加を選び、よむで学習対象に日本語を選んでから、日本語のページを開きます。',
    ...
    'After choosing Japanese, the remaining defaults are enough for the first week. Do not connect five services. Do not import a shelf of dictionaries. Read one short thing, press a few words and come back tomorrow.': '学習対象に日本語を選んだ後は、残りの初期設定で最初の一週間を始められます。五つのサービスをつながないでください。辞書を棚いっぱい取り込まないでください。短いものを一つ読み、いくつかの単語を押し、明日また戻ります。',
    ...
    'Manga and games: text inside pictures →': '漫画とゲーム：画像の中の文字 →',
    'Manga and games': '漫画とゲーム',
    'Read text trapped inside manga panels, screenshots and game frames with OCR, while keeping image requests explicit.': '画像の読み取りを明示的な操作に保ちながら、OCRで漫画のコマ、スクリーンショット、ゲーム画面に閉じ込められた文字を読みます。',
    'Permalink to "Manga and games"': '「漫画とゲーム」へのパーマリンク',
    'Some writing is trapped in a picture.': '画像の中に閉じ込められた文字もあります。',
    ...
    'First choose the language you are reading under Settings → Appearance. Yomu does not assume Japanese on a fresh install. The examples below use Japanese manga and game dialogue, but the same capture loop follows whichever supported learning target you choose.': 'まず「設定」→「外観」で、読んでいる言語を選びます。新規インストールのよむは日本語を前提にしません。以下では日本語の漫画とゲームの台詞を例にしますが、同じキャプチャ手順は、選んだ対応済みの学習対象に従います。',
    'Permalink to "Read manga"': '「漫画を読む」へのパーマリンク',
    'Some Japanese manga pages ship recognised text beside the image, as Mokuro pages do. Yomu reads that embedded text immediately. Other pages and languages need an OCR provider after you ask for a scan.': 'Mokuroページのように、画像と一緒に認識済み文字を配信する日本語漫画のページがあります。よむはその埋め込み文字をすぐに読みます。ほかのページや言語では、スキャンを指示した後にOCRプロバイダーが必要です。',
    ...
    'Yomu Gaming is a separate desktop app for Windows, macOS, Linux and Steam Deck desktop mode. On first launch, choose the language you want to read before setting up or starting capture. Then choose a whole-screen or region capture shortcut and press it during a scene. Text in the selected learning target becomes the same pressable reading surface.': 'Yomu GamingはWindows、macOS、Linux、Steam Deckのデスクトップモード向けの別アプリです。初回起動時は、キャプチャを設定または開始する前に読みたい言語を選びます。その後、全画面または範囲キャプチャのショートカットを選び、場面の途中で押します。選んだ学習対象の文字が、同じ押せる読書画面になります。',
    ...
    'For Japanese, Yomu Hosted Audio provides the default recorded pronunciation. Other targets use speech synthesis in the selected language by default. Where available, you can enable source audio from Jiten, Bunpro and other connected providers, import custom JSON sources, or run Ultimate Yomitan Audio on your computer.': '日本語では、Yomu Hosted Audioが既定の録音発音を提供します。その他の学習対象では、選択した言語の音声合成を既定で使います。利用できる場合は、JitenやBunproなど接続した提供元の音声を有効にしたり、独自JSON出典を取り込んだり、コンピューターでUltimate Yomitan Audioを動かしたりできます。',
    ...
    "Read the language you're learning with Yomu.": '学んでいる言語を、よむで読む。',
    'Japanese': '日本語',
    'Chinese': '中国語',
    'Cantonese': '広東語',
    'Korean': '韓国語',
    ...
    'The desktop app reads your selected learning language on screen with OCR and hands it back as words you can press. Separate download for Windows, macOS, Linux and Steam Deck.': 'デスクトップアプリが画面上の選んだ学習言語をOCRで読み取り、押せる単語として返します。Windows、macOS、Linux、Steam Deck向けの別ダウンロードです。',
    ...
    'Tap a panel and よむ finds text in your selected learning language, on a laptop or with a thumb on an iPad.': 'コマをタップすると、よむが選んだ学習言語の文字を見つけます。パソコンでも、iPadなら指でも使えます。',
    ...
    'A reader that turns what you already read into language study.': 'いつも読んでいるものを、そのまま言語学習に変えるリーダーです。',
    'Press a word, anywhere.': 'どこでも、単語を押すだけ。',
    'Web pages, YouTube subtitles, manga pictures, PDFs — one press gives the meaning and the target-appropriate reading or pronunciation. Japanese additionally provides furigana, pitch accent and recorded audio.': 'ウェブページ、YouTubeの字幕、漫画の画像、PDF — ひと押しで意味と、その学習言語に合った読みや発音が出ます。日本語では、ふりがな、ピッチアクセント、録音音声も使えます。',
    ...
    'No. Install Yomu, choose the language you are learning, open a page in that language, and press a word — that is the whole setup. Connecting Anki, jpdb, Bunpro or WaniKani is optional and only for people who already use them.': 'いいえ。よむをインストールし、学んでいる言語を選び、その言語のページを開いて単語を押す — 準備はそれだけです。AnkiやjpdbやBunpro、WaniKaniとの連携は任意で、すでに使っている人のためのものです。',
    ...
    'Yes. On Android, install Firefox and add Yomu from its store. On iPhone and iPad, Yomu runs inside Safari — lookup, reviews, and manga reading all work by touch; Japanese also gets furigana and pitch accent.': 'はい。AndroidではFirefoxを入れて、そのストアからよむを追加します。iPhoneとiPadではSafariの中で動き、検索、復習、漫画の読み取りまでタッチで使えます。日本語では、ふりがなとピッチアクセントも使えます。',
    ...
    'Any page with text in your selected learning language. On top of that, YouTube gets its own subtitle reader with the video, image-based manga readers work through picture reading, and there is a': '選んだ学習言語の文章があるページならどこでも。さらにYouTubeには動画と並ぶ専用の字幕リーダーがあり、画像型の漫画リーダーは画像読み取りで対応します。手元のファイルには',
    'and a': 'と',
    'for your own files.': 'があります。',
    'How does it read manga and pictures?': '漫画や画像はどうやって読むのですか？',
    'Press a picture — or use the Scan images command — and Yomu recognises text in your selected learning target, so every recognised word becomes a word you can press. Recognition uses Google Lens by default, with no key or account; you can switch to your own Google Cloud Vision key, or to a fully local service, in Settings.': '画像を押すか「画像をスキャン」コマンドを使うと、よむが画像内にある現在学習中の言語のテキストを認識し、認識された単語をすべて押せるようにします。認識は初期設定でGoogle Lensを使い、キーもアカウントも不要です。設定で自分のGoogle Cloud Visionキーや、完全にローカルなサービスにも切り替えられます。',
    ...
    'Word colours show how well you know them when a review system is connected, so a page shows you at a glance what is new and what is due. For Japanese, underline colours can also show pitch-accent patterns. All of it can be turned off in Settings.': '復習システムを連携すると、単語の色で習熟度を表し、新しい単語と復習期限の単語がひと目でわかります。日本語では、下線の色でピッチアクセントの型も表示できます。すべて設定でオフにできます。',
    'No — all 33 targets can be read, mined and reviewed. First-run setup requires you to choose one rather than assuming Japanese. You can look a word up, keep it with the sentence where you found it, and review it on a schedule in any target; the dictionary catalogue carries headwords across all of them.': 'いいえ。33の学習言語すべてで、読む・集める・復習するができます。初回セットアップでは日本語を前提にせず、学習言語を明示的に選びます。どの言語でも、単語を調べ、見つけた文と一緒に保存し、スケジュールに沿って復習できます。辞書カタログには全対象言語の見出し語があります。',
    ...
    'Uchisen is now an optional outbound lookup link only. よむ no longer downloads or displays Uchisen pages, mnemonic stories, images, keywords, or components, and no longer sends image-generation or mnemonic-publishing requests. No imported or learner-owned data is deleted.': 'Uchisenは任意の外部検索リンクのみになりました。よむはUchisenのページ、ニーモニック文、画像、キーワード、構成要素をダウンロードまたは表示せず、画像生成やニーモニック公開のリクエストも送信しません。取り込んだデータや学習者が所有するデータは削除しません。',
    'Browser-store and hosted-app descriptions, PWA identities, and stable install links now present よむ as a 33-target language reader instead of a Japanese-only tool. Japanese remains the deepest target, but it is no longer treated as the default product identity.': 'ブラウザーストアとホスト版アプリの説明、PWAの製品名、安定したインストールリンクは、よむを日本語専用ツールではなく、33の対象言語に対応するリーダーとして案内するようになりました。日本語は引き続き最も充実した対象言語ですが、製品全体の既定の前提としては扱いません。',
    ...
    "Use the Yomu button on any page. On ordinary websites, Open settings launches the Yomu-owned Study page so the host cannot read or rewrite credentials, imports, recovery codes, or the values you save. The full dialog opens directly on Study and the extension's new-tab page.": 'どのページでもよむのボタンを使えます。通常のウェブサイトで「設定を開く」を選ぶと、ホスト側から認証情報、インポート、復旧コード、保存する値を読み取ったり書き換えたりできないよう、よむが所有するStudyページを開きます。完全なダイアログはStudyと拡張機能の新規タブページで直接開きます。',
    'Fresh setup is language-neutral. Target-specific reading and Japanese-only preferences stay inactive until you explicitly choose a learning target. Some compatibility fields retain historical stored values; when one could look like a fresh Japanese default, the table states the effective gated behavior instead.': '新規セットアップは特定の言語を前提にしません。学習対象を明示的に選ぶまでは、対象言語に固有の読解機能と日本語専用の設定は動作しません。互換性のため過去の保存値を保持する項目もあります。新規環境で日本語が既定で有効になっているように見える場合は、表に実際の無効化状態を示します。',
    'Each row gives the label the dialog shows, the explanation the dialog offers, the stored default or effective fresh-install gate, and the name the setting takes in an exported settings file.': '各行には、ダイアログに表示されるラベル、ダイアログの説明、保存上の初期値またはインストール直後の有効化条件、設定エクスポートでの名前を載せています。',
    ...
    'Selected learning-language text on webpages': '選択した学習言語のウェブページ上のテキスト',
    'inactive until a learning target is explicitly chosen': '学習対象を明示的に選ぶまで無効',
    'Filter YouTube to the selected learning language': 'YouTubeを選択した学習言語に絞る',
    'stored on; inactive before target choice, then automatic for Japanese or opt-in for any other target': '保存値はオン、学習対象の選択前は無効です。日本語では選択後に自動で有効になり、その他の対象では明示的に有効化できます。',
    'stored on; inactive until Japanese is explicitly chosen': '保存値はオン、日本語を明示的に選ぶまで無効',
    'off; explicit opt-in after choosing Japanese': 'オフ、日本語選択後に明示的な有効化が必要',
    'Filter YouTube for the selected learning language. Japanese channel suggestions and Japanese-site navigation are available only after Japanese is explicitly chosen.': 'YouTubeを選択した学習言語に絞ります。日本語チャンネルの候補と日本語版サイトへの移動は、日本語を明示的に選んだ後にのみ利用できます。',
    'Support': 'サポート',
    'Stats': '統計',
    'Changelog': '変更履歴',
    'Added': '追加',
...
const HOSTED_DOCS_COPY_BY_LANGUAGE: Readonly<Record<ReviewedDocsLocale, Readonly<Record<string, string>>>> = Object.freeze({
    en: HOSTED_DOCS_EN_COPY,
    ja: HOSTED_DOCS_JA_COPY,
});

export const REVIEWED_DOCS_MESSAGES: readonly ReviewedDocsMessage[] = reviewedDocsMessages();

function reviewedDocsMessages(): readonly ReviewedDocsMessage[] {
    const messages = new Map<MessageId, ReviewedDocsMessage>();
    for (const [en, ja] of Object.entries(HOSTED_DOCS_JA_COPY)) {
        const id = reviewedDocsMessageId(en);
        if (!messages.has(id)) messages.set(id, Object.freeze({ id, en: en.trim(), ja }));
    }
    return Object.freeze([...messages.values()]);
}

function reviewedDocsMessageId(source: string): MessageId {
    return `${legacyDocsMessageId(source)}-${secondaryDocsHash(source)}` as MessageId;
}

function secondaryDocsHash(source: string): string {
    let hash = 5381;
    for (const character of source.trim()) hash = ((hash * 33) ^ character.codePointAt(0)!) >>> 0;
    return hash.toString(16).padStart(8, '0');
}

export function translateReviewedDocsText(value: string, locale: ReviewedDocsLocale): string {
    const parts = splitReviewedDocsText(value);
    const translated = HOSTED_DOCS_COPY_BY_LANGUAGE[locale][parts.core];
    if (!translated) return value;
    return `${parts.leading}${translated}${parts.trailing}`;
}

export function hasReviewedDocsText(value: string): boolean {
    const core = splitReviewedDocsText(value).core;
    return Boolean(HOSTED_DOCS_JA_COPY[core] ?? HOSTED_DOCS_EN_COPY[core]);
}

function splitReviewedDocsText(value: string): { leading: string; core: string; trailing: string } {
