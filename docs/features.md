...
Mobile handoff is deliberately narrower. On iPhone, iPad, and Android, よむ can open AnkiMobile or AnkiDroid to create a new note when AnkiConnect is not available. Handoff does not scan existing decks, read your collection, pull existing-card status, update existing notes, adapt to existing decks, or provide review queues. For full Anki on mobile, keep desktop Anki open with [AnkiConnect](https://ankiweb.net/shared/info/2055492159) and point よむ at that desktop through Wi-Fi or [Tailscale](https://tailscale.com/downloads).
