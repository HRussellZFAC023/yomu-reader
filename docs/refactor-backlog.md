...
- UT-34 shortcut hint labels: stop permanently showing keyboard hints like the "Space" pill under the study Hide/Reveal button — shortcuts are discoverable in settings. Middle ground accepted by user: show a hint until the shortcut is used once, then drop it. Also: never show keyboard hints on touch-only devices (no keyboard). Audit ALL surfaces with inline shortcut labels (study page reveal/grade buttons, subtitle panel, popup).
- UT-35 swipe-to-grade must be toggleable in settings (check whether a setting already exists; if not, add one — study page settings section — default on; the gesture shipped in 0.6.146).
