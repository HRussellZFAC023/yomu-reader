...
```mermaid
flowchart LR
  Need["A person needs the Japanese"] --> Input["See or hear it in context"]
  Input --> Teach["Concise explanation + worked example"]
  Teach --> Guide["Guided manipulation"]
  Guide --> Produce["Recall, speak, write, listen, or act"]
  Produce -->|wrong| Repair["Exact contrast + smaller retry"]
  Repair --> Produce
  Produce -->|ready| Transfer["Use it in a changed scene"]
  Transfer --> Close["Scene resolves; evidence is saved"]
  Close --> Review["Later Study and world encounters"]
```
