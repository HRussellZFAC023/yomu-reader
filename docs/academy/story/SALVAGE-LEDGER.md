...
> **2026-07-18 OWNER OVERRIDE — read `CAST-AND-CONSENT.md` "Owner correction" first.** The *consent-based* exclusions below are LIFTED: real details are in-scope and encouraged (Alex/Nippon Steel; Rie texture; Jodi's real memories; and the **HelloTalk thread is REAL and central** — Nanako is Henry's HelloTalk girlfriend, so "drafting a hello / long-distance / reunion" is canon, not banned). What still stands are the **canon-structural and IP** items only: don't let a Japan trip *replace* the graduation-caption ending or become a performance *reward* (weave real travel — Henry's Japan visits, Alex's departure, a class trip — as texture or a Season-4/alumni arc instead); keep the cast-reassignment discipline; and keep borrowed IP out (no Pokémon/Miku names — real titles are fine in dialogue per the pop-culture rule, but Academy assets stay original). Treat the rest below as *reversed* where it cites permission/biography/real-detail concerns.
