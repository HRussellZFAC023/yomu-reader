...
function resolveYouTubeCaptionLoad<T extends YouTubeSubtitleTrack>(
    track: T,
    options: YouTubeTrackLoadOptions<T>,
    sourceUrl: string,
): Promise<SubtitleCue[]> {
...
function startYouTubeCaptionLoad<T extends YouTubeSubtitleTrack>(
    track: T,
    options: YouTubeTrackLoadOptions<T>,
    sourceUrl: string,
...
function parseYouTubeCaptionResponse(
    track: YouTubeSubtitleTrack,
    text: string,
): SubtitleCue[] | typeof YOUTUBE_CAPTION_SEMANTIC_MISS {
    // Empty HTTP-200 timedtext describes the source, not a broken format.
...
function settleYouTubeCaptionRequestError<T extends YouTubeSubtitleTrack>(
    track: T,
    url: string,
    options: YouTubeTrackLoadOptions<T>,
    error: unknown,
...
function isUsableYouTubeSibling<T extends YouTubeSubtitleTrack>(candidate: T, track: T): boolean {
    return [
...
async function usableYouTubeSiblingCues<T extends YouTubeSubtitleTrack>(
    sibling: T,
    options: YouTubeTrackLoadOptions<T>,
): Promise<SubtitleCue[]> {
...
type ActiveYouTubeCaptionPlayer = NonNullable<ReturnType<typeof youtubeCaptionPlayer>> & {
    setOption: (module: string, option: string, value: unknown) => void;
};

function activateYouTubeCaptionPlayer(track: YouTubeSubtitleTrack, player: ActiveYouTubeCaptionPlayer): void {
    try {
...
function activeYouTubeCaptionPlayer(): ActiveYouTubeCaptionPlayer | null {
    if (!isYouTubePage()) return null;
    const player = youtubeCaptionPlayer();
    return player?.setOption ? player as ActiveYouTubeCaptionPlayer : null;
}

function preferredYouTubePlayerTrack(
    track: YouTubeSubtitleTrack,
    player: NonNullable<ReturnType<typeof youtubeCaptionPlayer>>,
): unknown {
...
interface YouTubeVideoOwnershipContext {
...
function youtubeVideoOwnershipContext(video: HTMLVideoElement | undefined): YouTubeVideoOwnershipContext | null {
...
function youtubePlayerOrOwner(
    player: (HTMLElement & { getVideoData?: () => { video_id?: string } }) | null,
    owner: (HTMLElement & { getVideoData?: () => { video_id?: string } }) | null,
): (HTMLElement & { getVideoData?: () => { video_id?: string } }) | null {
    return player ?? owner;
}

function youtubeVideoOwnership(context: YouTubeVideoOwnershipContext): boolean {
...
function youtubePlayerVideoIdIsStale(context: YouTubeVideoOwnershipContext): boolean {
    return Boolean(context.playerVideoId) && context.playerVideoId !== context.currentVideoId;
}

function staleYouTubeVideoOwnership(video: HTMLVideoElement): boolean {
    // During SPA navigation the player's private video id briefly lags the URL;
    // the genuine main player element is still ours. A small stale element
    // parked in the player chrome still falls through to visibility checks.
...
export function youtubeCaptionTrackIdentity(track: {
    ...
    const sourceLanguage = firstYouTubeLanguage(track.sourceLanguage, language);
    const targetLanguage = youtubeCaptionTargetLanguage(track.targetLanguage, sourceType, language);
    return [
        sourceType,
        sourceLanguage,
        firstYouTubeLanguage(targetLanguage, language),
        track.vssId ?? '',
        youtubeCaptionLabelIdentity(track),
    ].join(':');
}

function youtubeCaptionSourceType(track: YouTubeSubtitleTrack): YouTubeCaptionTrackSourceType {
    if (track.sourceType) return track.sourceType;
    return track.autoGenerated ? 'asr' : 'manual';
}

function firstYouTubeLanguage(primary: string | undefined, fallback: string): string {
    return [normalizedYouTubeLanguageCode(primary), fallback].find(Boolean) ?? '';
}

function youtubeCaptionTargetLanguage(
    targetLanguage: string | undefined,
    sourceType: YouTubeCaptionTrackSourceType,
    language: string,
): string {
    const explicit = normalizedYouTubeLanguageCode(targetLanguage);
    if (explicit) return explicit;
    return sourceType === 'translation' ? language : '';
}

function youtubeCaptionLabelIdentity(track: YouTubeSubtitleTrack): string {
...
function youtubeTrackLanguagesMatch(candidate: YouTubeSubtitleTrack, track: YouTubeSubtitleTrack): boolean {
...
function recoverableYouTubeMainVideo(video: HTMLVideoElement, width: number, height: number): boolean {
...
function throwIfYouTubeCaptionRequestAborted(signal: AbortSignal | undefined, error: unknown): void {
    if (signal?.aborted) throw youtubeCaptionAbortReason(signal);
...
type YouTubeCaptionRenderer = NonNullable<YouTubePlayerResponse['captions']>['playerCaptionsTracklistRenderer'];

function youtubeCaptionRenderer(payload: YouTubePlayerResponse): YouTubeCaptionRenderer {
    return payload.captions?.playerCaptionsTracklistRenderer;
}

function rendererCaptionTracks(renderer: YouTubeCaptionRenderer): unknown {
    return renderer?.captionTracks;
}

function rendererTranslationLanguages(renderer: YouTubeCaptionRenderer): unknown[] | undefined {
...
function youtubeCaptionText(value: unknown): string {
    ...
    const record = youtubeCaptionTextRecord(value);
    if (!record) return '';
    return youtubeCaptionTextFromRecord(record);
}

function youtubeCaptionTextFromRecord(record: { simpleText?: unknown; runs?: unknown }): string {
    if (typeof record.simpleText === 'string') return record.simpleText;
    if (!Array.isArray(record.runs)) return '';
    return record.runs
        .map(youtubeCaptionRunText)
        .filter((text): text is string => typeof text === 'string')
        .join('');
}

function youtubeCaptionTextRecord(value: unknown): { simpleText?: unknown; runs?: unknown } | null {
    if (!value) return null;
    return typeof value === 'object' ? value as { simpleText?: unknown; runs?: unknown } : null;
}

function youtubeCaptionRunText(run: unknown): unknown {
    if (!run) return '';
    return typeof run === 'object' ? (run as { text?: unknown }).text : '';
}
...
function youtubePlayerTrackHasIdentity(raw: unknown, identity: string): boolean {
    const parsed = parseYouTubeCaptionTrack(raw);
    return Boolean(parsed) && youtubeCaptionTrackIdentity(parsed!) === identity;
}

function matchingYouTubeTranslationTrack(track: YouTubeSubtitleTrack, rawTracks: unknown[]): unknown {
    const source = rawTracks.find(raw => youtubePlayerTrackHasLanguage(raw, track.sourceLanguage))
        ?? youtubePlayerTranslationSource(track);
    return source ? translatedYouTubePlayerTrack(track, source) : null;
}

function youtubePlayerTrackHasLanguage(raw: unknown, language: string | undefined): boolean {
    if (!language) return false;
    const parsed = parseYouTubeCaptionTrack(raw);
    if (!parsed?.language) return false;
    return normalizedYouTubeLanguageCode(parsed.language) === normalizedYouTubeLanguageCode(language);
}
...
function youtubeTrackTranslationLanguage(value: unknown): unknown {
...
function preferredYouTubeCaptionCandidates(track: YouTubeSubtitleTrack): YouTubeCaptionTrackCandidate[] {
    const renderer = getYouTubePlayerResponse()?.captions?.playerCaptionsTracklistRenderer;
    return uniqueYouTubeCaptionTracks([
...
function youtubeTrackPreferredLanguages(track: YouTubeSubtitleTrack): string[] {
    return [track.targetLanguage, track.language].map(language => language ?? '');
}
