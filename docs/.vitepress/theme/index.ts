...
const HOSTED_DOCS_JA_COPY: Record<string, string> = {
    ...
    'Fixed': '修正',
    'Kept compact host UI labels such as author names, usernames, metadata, and headers passive so Yomu annotations preserve the site\'s native text color and avoid intrusive highlight fills, while normal chat/message prose still receives ruby.': '著者名、ユーザー名、メタ情報、ヘッダーなどのコンパクトなサイトUIラベルをパッシブ扱いにし、Yomuの注釈がサイト本来の文字色を保ち、目立ちすぎるハイライト塗りを避けるようにしました。一方で通常のチャットやメッセージ本文には引き続きルビが付きます。',
