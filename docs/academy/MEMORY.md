...
- `<PRIVATE_CLASS_INVITE>` is now a real Cloudflare/D1 session path. The approved Persona and Shinday files stream only through authenticated R2 routes; anonymous media is 401 and the service worker never caches protected media.
...
- The current checkpoint is rebased on Yomu 1.6.149, pushed through `798465abe`, and live at `s1-49196f5d199d`: Reader CI passes 3,583 tests with one skip plus all 1,010 JPDB cases; focused Study/offline coverage passes 406/406; Academy passes 75 files / 428 tests; deterministic QA passes 13/13 and docs accessibility 66/66; source/library, dead-code, exact line-scoped Fallow, complexity, canonical builds, VitePress, userscript verification, and Chrome/Firefox/Safari extension packaging pass. Worker version `b10096ab-dc6c-42d2-a48e-d3dd5427a269` serves live `<PRIVATE_CLASS_INVITE>` sessions and authenticated Persona/Shinday media. Resumed Fable-low session `b29c8446-9c18-4a8e-b5a2-93095bc02f5f` returned `PASS` with no blocker. It remains 0/73 playable Weeks by design.
