...
const HOSTED_DOCS_JA_COPY: Record<string, string> = {
    ...
    "Release packaging now uses the same eight smaller regular-test shards as CI. The release runner had folded those tests into four shards twice the size, and its second long-lived jsdom process repeatedly died without an assertion or summary even though every test passed in CI; smaller serial shards keep the same isolation and coverage without exhausting the runner before it can report a result.": "リリースのパッケージ作成でも、CIと同じ8つの小さな通常テストシャードを使うようになりました。これまではリリース用ランナーだけがテストを2倍の大きさの4シャードにまとめており、CIでは全テストが通っているにもかかわらず、2番目の長時間動作するjsdomプロセスがアサーションも結果の要約も出さずに繰り返し終了していました。小さなシャードを直列に実行することで、同じ分離と網羅性を保ちながら、結果を報告する前にランナーの資源を使い切ることを防ぎます。",
    ...
    'Reader-account devices and Academy profiles now share one bounded encrypted export without mixing their independent event cursors. The export freezes both logs, streams Academy then Reader history, includes revocable Reader credential metadata without bearer secrets, and counts both logs in deletion receipts.': 'Readerアカウントの端末とAcademyプロフィールは、独立したイベントカーソルを混同せず、上限付きの暗号化エクスポートを1つ共有するようになりました。エクスポートは両方のログを固定し、Academy、Readerの順に履歴をストリーム出力します。ベアラーシークレットを含まない失効可能なReader認証情報のメタデータを含め、削除証明では両方のログを集計します。',
    "Aakash's approved v009 anime-style sprite family is now the runtime source across Academy surfaces; the superseded v005 family is archived outside the repository rather than retained as an active candidate.": 'Aakashが承認したv009のアニメ風スプライト一式を、Academyの各画面で実行時の正式な素材として使用するようになりました。置き換えられたv005一式は候補としてリポジトリに残さず、リポジトリ外へアーカイブしました。',
    'The Academy production plan now treats the calendar as Day 1 through Day N. A day closes only after every required, optional, revisitable, one-off, social, study, exploration, minigame, and evening activity available on that date is implemented and verified; the 48 core chapters do not cap the calendar.': 'Academyの制作計画では、カレンダーをDay 1からDay Nまでとして扱うようになりました。その日に利用できる必須、任意、再訪可能、1回限り、交流、学習、探索、ミニゲーム、夜の各アクティビティをすべて実装・検証して初めて、その日を完了とします。48の主要チャプターはカレンダーの上限ではありません。',
    ...
    'Academy lifecycle proof deletion now requires an expiring, single-use server grant bound to the authenticated production test account and run nonce. The supervised proof compares the active immutable Worker version and script digest with a locally reproduced reviewed bundle, exports large encrypted histories through a bounded stream, protects export creation as a same-origin POST, and prunes 90-day receipts on an observable scheduled retry path.': 'Academyのライフサイクル検証で削除を行うには、認証済みの本番テストアカウントと実行nonceに紐付いた、有効期限付き・1回限りのサーバー許可が必要になりました。監督下の検証では、稼働中の不変なWorkerバージョンとスクリプトのダイジェストを、ローカルで再現したレビュー済みバンドルと比較します。また、大規模な暗号化履歴を上限付きストリームで出力し、エクスポート作成を同一オリジンのPOSTで保護し、監視可能な定期再試行経路で90日間の受領記録を整理します。',
    'Testing': 'テスト',
    'Academy': 'アカデミー',
