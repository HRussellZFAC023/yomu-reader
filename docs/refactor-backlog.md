...
- UT-25 ruby + colorising still not available immediately after page load (long-standing; persists in 0.6.144 per user). REPRO NOTES 2026-06-12 (anonymous chromium harness `/tmp/yt-repro.mjs`, search-results page): first annotation ~0.7-0.8s after script start; in-viewport titles all annotate after scroll-through (0 unannotated in viewport), so the scanner is viewport-gated by design — the cold-start complaint likely concerns the signed-in HOME rich-grid + userscript-manager boot timing on iPad. Needs the signed-in MCP browser for a faithful measure.
- UT-26 grid gaps persist: home grid rows with missing tiles (red-box screenshot), Shorts shelf sometimes renders a single item. Continuation/backfill pacing or our hiding/rebalance leaving holes. BLOCKED on signed-in MCP browser (anonymous home has no rich grid; search results showed 0 hidden/empty tiles).
