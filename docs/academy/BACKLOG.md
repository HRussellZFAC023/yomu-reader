...
**Active task:** `day:1:lesson-zero:07` - finish the first sentence-frame route as a complete learner journey: introduce each useful shape in Rie's dialogue before reconstruction, keep kana and English support earned rather than assumed, repair only the failed frame, require changed-turn transfer, schedule one stable SRS identity per learned shape, use the accepted Rie/Sophie lines, and prove story reachability, pause/reload persistence, responsive accessibility and return from the canonical onboarding path.
