...
`ledger` generates `@workflow-state/production-ledger.json`. It is the sole machine-readable delivery view: one row per canonical task records audit, implementation, learner reachability, QA and deployment evidence, while its progress percentages are derived from those same rows. Evidence states count only a proof whose exact file hash is pinned by a promotion and whose task-bound claim is still live or checkpointed; draft, expired, cross-task and merely self-labelled proof files cannot inflate the totals. Route counters are generated from named source registries and carry an explicit claim type so an authored file count cannot be presented as learner reachability. `status` reads this generated ledger rather than calculating a second percentage.
