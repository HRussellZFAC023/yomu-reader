...
| Full repository check | Amber for current delta | Academy passes 74 files / 400 tests; compound/expression/JPDB passes 1,023 tests; source/library validators, TypeScript, Academy build, docs build, hosted sync, and userscript verification pass. The readable userscript is 1,913,406 bytes (86,594 bytes below the 2 MB limit). Full `npm run check` / `npm run qa` remains open before release. |
