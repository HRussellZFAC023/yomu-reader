...
const HOSTED_DOCS_JA_COPY: Record<string, string> = {
    ...
    'Install a dictionary from the catalogue or import any compatible Yomitan ZIP. The starter uses both choices in your language profile: an English-speaking learner reading Spanish gets Spanish-headword terms with English definitions and Spanish IPA. Japanese terms, kanji and pitch remain the starter only when Japanese is the selected target. Dictionary files, search indexes and local lookup results stay in the browser. Reorder sources so the answer you trust appears first.': 'カタログから辞書をインストールするか、対応するYomitan ZIPを取り込みます。初期辞書は、言語プロフィールで選んだ二つの言語に合わせて決まります。たとえば、英語を使う学習者がスペイン語を読む場合は、スペイン語の見出し語、英語の語義、スペイン語のIPAが入ります。日本語を対象に選んだ場合は、日本語の単語、漢字、ピッチ辞書が初期設定になります。辞書ファイル、検索索引、端末内の検索結果はブラウザーに残ります。信頼する答えが先に出るよう、出典の順番を変えます。',
    ...
    ', and the dictionary catalogue carries headwords across all of them. Your recommended starter follows the selected target and definition language; for English plus Spanish, that means Spanish terms with English definitions and Spanish IPA. The interface itself speaks English and 日本語.': 'と表示され、辞書カタログにはその全言語の見出し語があります。おすすめの初期辞書は、選んだ対象言語と語義の言語に合わせて変わります。英語とスペイン語なら、スペイン語の単語、英語の語義、スペイン語のIPAが入ります。インターフェースは英語と日本語に対応しています。',
