...
const HOSTED_DOCS_JA_COPY: Record<string, string> = {
    ...
    'JPDB review questions no longer mistake sentence-side `.plain` tokens for the reviewed headword, so definitions and Immersion media stay hidden until the answer is revealed.': 'JPDBの復習問題で、文中の`.plain`トークンを復習対象の見出し語と誤認しなくなりました。定義とImmersionメディアは、答えを表示するまで隠れたままです。',
