...
function restoreLocalFallbackStoredState(key, state2) {
  try {
  if (state2.serializedValue === null) localStorage.removeItem(key);
  else localStorage.setItem(key, state2.serializedValue);
  if (localStorage.getItem(key) !== state2.serializedValue) throw new Error("read-back did not match");
  } catch (error) {
  throw storageWriteError(key, "Local fallback restore failed", error);
  }
  updateProvenance(key, state2.provenance, true);
}
...
function mirrorContentTokenForRecords(id, lookup) {
  ...
  const record2 = lookup(id);
  const fingerprint2 = record2 ? operationContentFingerprint(id, record2) : "";
  return fingerprint2 ? `o:${mirrorTokenHash(fingerprint2)}` : "";
}
