...
**Current delivery checkpoint:** the delivery tree is synchronized through Reader `v1.7.0`; shared story, audio, N3, account lifecycle, workflow, retained upstream work, and the approved Aakash `v009` runtime promotion are consolidated. The executable Day 1 availability manifest enumerates 52 learner-facing surfaces and 260 closure proofs across implementation, reachability, media, persistence, and full-journey evidence. The class-code entrance, linked-account lifecycle, installed offline welcome, learner profile, first Rie greeting, starting-path choice, vowel listening, repeatable listening bingo, five-kana writing, classroom-instruction rehearsal, understanding/repair rehearsal, living desk-language lesson, five connected first-sentence frames, saved-name `です` transfer, the two-speaker name-listening mission, the reversible manual-level entry, and the accepted placement path now account for 67 verified dimensions, 23 partial dimensions, and 170 unverified dimensions. The access route keeps invalid and temporarily unavailable codes editable, prevents duplicate exchanges, cancels late requests when the screen leaves, reaches the Google account fork, persists the accepted invite, and resumes that exact fork without spending the code again. Its real HTTP gateway journey, accessible repair states, reduced-motion behavior, geometry, touch targets, and reload recovery are browser-proved on phone, portrait tablet, and desktop. The linked-account journey then proves Google handoff, first-device encrypted profile setup, refresh, Reader-device listing, encrypted pairing, downloaded export, session-family sign-out, recovery without another invitation, and explicit account deletion with local-key cleanup at the same three viewports. The installed build now caches every required Reader companion, explains offline state without blocking the scene, commits learner evidence locally, encrypts its pending queue, cold-restores the exact Rie checkpoint from the production service worker, and refreshes account authority before flushing that queue exactly once on reconnect. Its layout, touch targets, serious/critical Axe surface, no-second-invite invariant, and phone/tablet/desktop visual composition are browser-proved. The learner profile is a short Rie conversation rather than a personal-facts form: it asks for one ordinary display name, one concrete thing the learner wants to do in Japanese, and one story portrait. English support is visible before a zero-kana learner acts; the saved event contains the chosen name, goal, and portrait exactly once; and the real enrollment route, IndexedDB event, keyboard/touch flow, responsive geometry, accessibility surface, and cold reload into Rie's first scene are browser-proved on phone, portrait tablet, and desktop. Rie's first meeting now uses the canonical Chapter 1 foundation line, immediate English support, her approved classroom performance, and its exact deterministic Aivis Opus voice. The profile alone cannot claim the meeting; entering records its encounter, scene, character, relationship, and journal evidence exactly once. Reload skips the completed one-off, while an intentional journal revisit replays the same scene and voice without duplicating progress. That entire contract, responsive geometry, touch targets, accessibility surface, audio request, and visual composition are browser-proved on phone, portrait tablet, and desktop. The starting-path screen now asks one plain question, explains each option without assuming Japanese knowledge, keeps the opening music and interaction cues, prevents duplicate activation, persists the chosen branch, and cold-restores it without falsely assigning a level. Lesson Zero records its curriculum entry once; manual-band and placement remain reversible until their own result is accepted. All three branches, responsive geometry, decoded destination art, touch targets, accessibility surface, Back behavior, and reload recovery are browser-proved on phone, portrait tablet, and desktop. The manual-level path now offers five plain self-assessments from foundations through dense Japanese, keeps the invitation theme and interaction cues, prevents duplicate activation, and records only an intentional choice. Every level reaches the right learner-facing arrival without exposing production provenance or later Day 1 work; Back remains available, a deliberate level change preserves honest history while the latest choice wins, and two cold reloads restore that projection exactly. The N3 listening bridge also keeps Rie visibly staged and readable through its longer activity instead of stretching her portrait below the fold. All 15 band journeys, responsive geometry, decoded art, accessibility, persistence, reversibility, and clean consoles are browser-proved across phone, portrait tablet, and desktop. The name-card draft reuses that saved learner name, asks for one N+1 ordering move, records repair and one canonical SRS seed, resumes durably, and is keyboard/touch accessible and browser-proved at the same three viewports. It does not duplicate name capture or pretend a Lesson Zero beginner can transcribe an arbitrary name into katakana; that skill remains in the adaptive kana game and later name-card activities. The name-listening mission teaches one useful sound landmark before testing, plays exact Aivis Opus lines, withholds transcripts until commitment, records a deliberate lapse and canonical SRS evidence, replays only the missed voice, survives Back and reopen, satisfies the authored story gate, returns to the exact cursor, and resumes with Mika visibly speaking on phone, portrait tablet, and desktop. The placement path now gives Rie's briefing before any scored item, uses six level-bounded N+1 questions plus speaking and writing production, requires audio or an explicit transcript alternative, cold-restores every draft step, withholds learner evidence until acceptance, and keeps Back and manual override reversible. Its packaged N5 listening asset, production alternatives, responsive living-paper result, touch and keyboard controls, Axe surface, accept-once contract, override-without-evidence contract, and cold resume are browser-proved on phone, portrait tablet, and desktop. Strict Day 1 closure is therefore `9/52`; partial evidence never counts as a completed activity. The active task is `day:1:arrival`: carry the accepted starting choice into a canonical courtyard arrival without teleporting, duplicating onboarding, losing route history, or breaking offline resume; bind the scene's voice, art, music, ambience, SFX, first-day chronology, and phone, portrait-tablet, and desktop journey proof.
