...
**Current delivery checkpoint:** the delivery tree is synchronized through Reader `v1.7.0`; shared story, audio, N3, account lifecycle, workflow, retained upstream work, and the approved Aakash `v009` runtime promotion are consolidated. The executable Day 1 manifest contains 52 learner-facing surfaces and 260 closure proofs. It currently records **72 verified**, **23 partial**, and **165 unverified** dimensions; strict Day 1 closure is **`10/52`**, because partial evidence never counts as completion.

**Closed Day 1 journeys:** class-code access, linked-account lifecycle, installed offline resume, learner identity, Rie's first meeting, starting-path choice, reversible manual-level entry, accepted placement, the canonical Academy arrival, and the two-name listening mission. Their production routes persist exact progress and are covered by automated contracts plus real phone, portrait-tablet, and desktop journeys.

**Latest closure - `day:1:arrival`:** Lesson Zero, every manual band, and accepted placement now converge on one authored three-scene arrival without teleporting, repeating onboarding, losing Back history, or inventing Japanese skills for a zero-kana learner. The route cold-restores its exact cursor, records scene and completion evidence once, carries opening art/music/ambience/SFX, and uses ten source-locked AivisSpeech Opus variants for Rie's five foundation/N5 lines. The full Academy suite is green, and the production journey is browser-proved across all three target viewports with serious/critical Axe checks, decoded visual assets, real audio requests, and clean consoles.

**Active task:** `day:1:blank-atlas` - complete Chapter 1 as the next chronological learner journey, including every story moment, lesson handoff, choice and repair, N+1 support, voice, art, music/SFX, persistence, return route, accessibility state, and phone/tablet/desktop proof.
