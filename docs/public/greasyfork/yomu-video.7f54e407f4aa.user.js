...
`);
      ...
      return `
        ...
        extraActions: `<button class="jpdb-subtitle-jump-current" type="button" data-action="jump-current" title="${escapeHtml(uiText(language, "jumpToCurrentSubtitle"))}" aria-label="${escapeHtml(uiText(language, "jumpToCurrentSubtitle"))}">${subtitleIcon("locate")}</button>`
            ...
            <div class="jpdb-subtitle-list-scroll" data-total-rows="${rowCount}"${state.virtual ? ' data-virtualized="true"' : ""}>
                ...
                ${state.rows.length ? state.rows.map((row, index) => this.renderRow(row, rowIndexOffset + index, state.currentRowIndex, transcriptRows)).join("") : this.renderWaitingState()}
  ...
  class SubtitlePlayerController {
    ...
    runtimeSignalsInitialized = false;
    videoResizeObserver;
    subtitleControlRail;
    ...
    installRuntimeDocumentObserver() {
      const settings = this.options.getSettings();
      const mode = this.video ? "full" : settings.subtitlePlayerEnabled ? "discovery" : "off";
      ...
      observer.observe(body, mode === "full" ? { attributeFilter: ["aria-modal", "class", "data-yomu-inline-fullscreen", "fullscreen", "hidden"], attributes: true, childList: true, subtree: true } : { childList: true, subtree: true });
      this.observer = observer;
    }
    handleRuntimeMutations(mutations) {
      if (this.destroyed) return;
      if (!this.video) {
    ...
    patchTranscriptVirtualWindow(state, scroller) {
      ...
      setInnerHtml(scroller, `
            ...
            ${state.rows.length ? state.rows.map((row, index) => this.transcriptPanelSurface.renderRow(row, rowIndexOffset + index, state.currentRowIndex, transcriptRows)).join("") : this.transcriptPanelSurface.renderWaitingState()}
