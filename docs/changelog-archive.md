...
Releases 1.7.6 and earlier, unedited. The current changelog is `CHANGELOG.md`, which carries 1.8.0 onward
and is what yomureader.com/changelog publishes.

Split out of `CHANGELOG.md` on 2026-08-04: that file is included verbatim into the site, and at 670,000
bytes across 995 releases it had become the heaviest page on yomureader.com.

This page is deliberately **not** routed by the site (`config/docs/published-pages.ts`). Old release notes
state what was true when they shipped -- language counts, a comparison route since retired, third-party
gaming OCR apps since replaced -- and publishing them as a live page would put those retired claims back
in front of crawlers. Read history here or in `git log`; do not edit it to match today.
