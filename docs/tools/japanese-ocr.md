...
- **A local OCR endpoint.** Point よむ at a local OCR app or server and it can recognize text on demand. It understands **MangaOCR**, **PaddleOCR**, Apple Vision-style wrappers, and compatible local JSON responses.

## Good for

- **Manga** — read raw Japanese manga panel by panel, checking only the words you don't know.
- **Screenshots & games** — use [Yomu Gaming](/tools/yomu-gaming) on PC, or send captured Japanese text through your configured OCR endpoint.
...
For PC games, download the first-party [Yomu Gaming release file](https://github.com/HRussellZFAC023/yomu-reader/releases/latest) and set the same local OCR endpoint there.
