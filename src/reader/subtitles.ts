...
interface ParseSubtitleTextOptions {
...
export class SubtitlePlayerController {
    ...
    private syncSubtitleSourceContext(video = this.video): boolean {
        ...
        log.debug('Subtitle source context changed', { previous, next: key });
        return true;
    }
    ...
    private removeSubtitleTracks(predicate: (track: SubtitleTrackOption) => boolean, reason: string): number {
        ...
        this.syncControls();
        log.debug('Subtitle tracks removed', { reason, removed: removed.length, remaining: this.tracks.length });
    ...
    private discoverPageSubtitleTracks(): void {
        ...
        for (const source of sources) {
            const existing = this.tracks.find(track => track.sourceKey === source.sourceKey || (track.url && sameSubtitleUrl(track.url, source.url)));
            if (existing) {
    ...
    private handleClick(event: MouseEvent): void {
        ...
        if (action === 'cue') this.seekToTranscriptRow(Number((event.target as HTMLElement).closest<HTMLElement>('[data-row-index]')?.dataset.rowIndex));
        if (action === 'previous') this.seekSubtitle(-1);
        if (action === 'next') this.seekSubtitle(1);
        if (action === 'copy') void this.copySubtitle();
        if (action === 'copy-row') void this.copyTranscriptRow(Number((event.target as HTMLElement).closest<HTMLElement>('[data-row-index]')?.dataset.rowIndex));
    ...
    private seekToTranscriptRow(index: number): void {
        ...
        log.debug('Subtitle transcript row selected', { index, start: row.cue.start, end: row.cue.end });
    }
    ...
    private renderTranscriptPanel(force = false): void {
        ...
        setInnerHtml(this.transcriptPanel, `
            ...
            <div class="jpdb-subtitle-list-scroll">
                ${rows.length
                    ? rows.map((row, index) => this.renderTranscriptRow(row, index, currentRowIndex)).join('')
        ...
        const worker = async () => {
            while (cursor < planned.length) {
                ...
                try {
                    ...
                    log.debug('Transcript background parse failed quietly', { rowIndex: item.rowIndex, length: item.text.length }, error);
                }
...
function mergeYouTubeCueFragments(current: SubtitleCue, next: SubtitleCue): SubtitleCue {
    ...
    const words = progressive
        ? nextWords
        : currentWords && (nextWords || allowsUntimedPunctuation)
            ? [...currentWords, ...(nextWords ? subtitleWordsAfterCompactOffset(nextWords, overlap) : [])]
            : undefined;
    return {
        ...current,
...
function parseYouTubeCaptionTrack(track: unknown): YouTubeCaptionTrackCandidate | null {
    ...
    const autoSuffix = autoGenerated && !/asr|auto(?:matic)?|auto-generated|自動生成|自動字幕/i.test(label)
        ? ' · auto-generated'
        : '';
