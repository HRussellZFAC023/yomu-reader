...
const HOSTED_DOCS_JA_COPY: Record<string, string> = {
    ...
    'Added a Steam Deck and PC gaming guide that replaces the YomiNinja-only workflow, explains the current PWA/no-install boundary, and compares Decky/native OCR helpers for game text handoff.': 'YomiNinjaだけに絞った従来のワークフローを置き換えるSteam Deck・PC向けゲームガイドを追加しました。現在のPWA／インストール不要ルートの境界を説明し、ゲームテキストの受け渡しに使えるDecky／ネイティブOCRヘルパーを比較しています。',
    ...
    'Read games with YomiNinja and よむ': 'YomiNinjaとよむでゲームを読む',
    'Open the gaming guide': 'ゲームガイドを開く',
    'YomiNinja releases': 'YomiNinjaリリース',
    'This YomiNinja-only guide has been replaced by the broader Steam Deck and PC gaming guide.': 'YomiNinjaだけに絞ったこのガイドは、より広いSteam Deck・PC向けゲームガイドに置き換わりました。',
    'YomiNinja is still useful on desktop: it extracts game or video text, overlays the recognized text, and supports browser dictionary extensions. For Steam Deck-first play, start with the new guide so you can choose between browser-readable text, Decky/native OCR helpers, and desktop mining tools without assuming YomiNinja is the only path.': 'YomiNinjaはデスクトップでは今でも有用です。ゲームや動画のテキストを抽出し、認識したテキストを重ねて表示し、ブラウザ辞書拡張にも対応します。Steam Deckを優先する場合は、YomiNinjaだけを前提にせず、ブラウザで読めるテキスト、Decky／ネイティブOCR補助ツール、デスクトップのマイニングツールから選べる新しいガイドから始めてください。',
    ...
    'A Decky or native OCR overlay such as Tango Lens or Decky Translator': 'Tango LensやDecky TranslatorのようなDecky／ネイティブOCRオーバーレイ',
    ...
    'YomiNinja, GameSentenceMiner, Kamite, Translumo, Kamui, Agent, Textractor, or a local OCR endpoint': 'YomiNinja、GameSentenceMiner、Kamite、Translumo、Kamui、Agent、Textractor、またはローカルOCRエンドポイント',
    ...
    'Tango Lens': 'Tango Lens',
    'Steam Deck-oriented OCR/lookup flow with Decky-style capture behavior.': 'Decky風のキャプチャ動作を持つSteam Deck向けOCR／検索フロー。',
    ...
    'Decky Translator': 'Decky Translator',
    ...
    'use Decky or a native helper for screen capture, global buttons, and overlays;': '画面キャプチャ、グローバルボタン、オーバーレイにはDeckyまたはネイティブ補助ツールを使います。',
    ...
    'On Steam Deck, expect Decky plugins and native helpers to have broader device access than a normal webpage.': 'Steam Deckでは、Deckyプラグインやネイティブ補助ツールが通常のWebページより広いデバイスアクセスを持つ前提で扱ってください。',
    ...
    'You want live lookup while playing: use a native overlay or Decky-style tool for the play session, then mine the good lines with よむ afterward.': 'プレイ中にライブ検索したい：プレイ中はネイティブオーバーレイまたはDecky風ツールを使い、後で良い行をよむでマイニングしてください。',
    'Open Study': '学習を開く',
    'OCR guide': 'OCRガイド',
    ...
    'That split matters. A browser or PWA is the cleanest path when another tool already exposes the text, but Steam Deck Game Mode capture and overlays still need a native, Decky, or packaged helper.': 'この分担が重要です。別のツールがすでにテキストを公開しているならブラウザやPWAが最もきれいな経路ですが、Steam Deck Game Modeのキャプチャとオーバーレイにはまだネイティブ、Decky、またはパッケージ化された補助ツールが必要です。',
    ...
    'The first Yomu-owned gaming app should be a Gaming Text Bridge rather than a full capture engine: a Steam Deck-friendly companion surface that accepts typed, pasted, OCR, clipboard, texthooker, or helper-app text and presents it as a normal よむ reader surface. Native capture can stay behind a Decky/Electron helper until a prototype proves it can be secure, simple, and maintainable.': 'よむが最初に自前で作るゲーム向けアプリは、完全なキャプチャエンジンではなくGaming Text Bridgeにするべきです。これは入力、貼り付け、OCR、クリップボード、テキストフッカー、補助アプリからのテキストを受け取り、通常のよむ読み取り面として表示するSteam Deck向けの補助面です。安全で単純かつ保守可能だとプロトタイプで示されるまでは、ネイティブキャプチャはDecky／Electronヘルパーの裏側に置けます。',
    ...
    'A free Migaku alternative': '無料のMigaku代替',
    ...
    'Where Migaku may suit you better': 'Migakuのほうが向いている場合',
    'A free alternative to Migaku for Japanese immersion': '日本語没入のためのMigakuの無料代替ツール',
    'Looking at': '検討しているのが',
    ...
    'Price': '価格',
    'Free': '無料',
    'Paid subscription — see migaku.com': '有料サブスクリプション — migaku.comをご覧ください',
    'Platform': 'プラットフォーム',
    'Browser userscript (desktop + iOS/iPad)': 'ブラウザのユーザースクリプト（デスクトップ + iOS/iPad）',
    'See migaku.com': 'migaku.comをご覧ください',
    ...
    'free, no account. Migaku is a separate paid, integrated suite; see migaku.com for its features and pricing.': '無料で、アカウントは不要です。Migaku は別の有料の統合スイートです。機能と価格については migaku.com をご覧ください。',
    'No, and it does not try to. It focuses on browser reading and mining with your own Jiten, JPDB, Anki and Yomitan dictionaries; Migaku is the all-in-one suite some people prefer.': 'いいえ、そしてそれを目指してもいません。ご自身の Jiten、JPDB、Anki、Yomitan 辞書を使った、ブラウザでの読書とマイニングに重点を置いています。Migaku はオールインワンのスイートで、それを好む人もいます。',
    'Next steps:': '次のステップ：',
    ...
    'OCR lets you look up Japanese text inside images. よむ can use embedded OCR metadata when a site provides it, or a local OCR app/server for MangaOCR, PaddleOCR, Apple Vision-style results, and compatible local JSON responses, including YomiNinja-style endpoints.': 'OCR機能により、画像内の日本語テキストを検索できます。よむは、サイトが提供している埋め込みOCRメタデータを使用するか、MangaOCR、PaddleOCR、Apple Vision形式の結果、YomiNinja形式のエンドポイントを含む互換ローカルJSONレスポンスのためのローカルOCRアプリ/サーバーを使用できます。',
