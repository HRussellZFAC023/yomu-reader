    ...
    it('idempotently claims a wrapped key, syncs opaque events, and revokes the bearer device', async () => {
        ...
        try {
            ...
            const firstExport = await (await dispatch(academy.env, request(
                academy.env, '/academy/api/account/export', 'POST', {}, ownerCookie,
            ))).json() as {
                eventPage: {
            ...
            expect(firstExport.readerSrsEventPage).toMatchObject({ events: [], hasMore: true });
            expect(firstExport.eventPage.exportCursor).toEqual(expect.any(String));
            const secondExport = await (await dispatch(academy.env, request(
                academy.env, '/academy/api/account/export', 'POST',
                { cursor: firstExport.eventPage.exportCursor }, ownerCookie,
            ))).json() as typeof firstExport;
            expect(secondExport.eventPage.events).toEqual([]);
            expect(secondExport.readerSrsEventPage).toMatchObject({
                events: [{ id: event.id }, { id: secondEvent.id }],
                hasMore: false,
            });
