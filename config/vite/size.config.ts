...
const artifactsRoot = process.env.YOMU_ARTIFACTS_ROOT
    ...
    : path.resolve(projectRoot, '..', '..', 'artifacts', 'yomu-reader');
...
function missingBaselineReport(): string {
    return `No baseline snapshot; copy ${path.relative(projectRoot, snapshotFile)} to ${path.relative(projectRoot, baselineFile)} to diff future runs.`;
}
