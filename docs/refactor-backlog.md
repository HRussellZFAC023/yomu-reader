...
- P1 (2026-06-10): Provider parity (Anki/JPDB/Jiten same abilities; audited 2026-06-10, corrected against passing smokes — Jiten new-tab queue and Jiten stats ARE supported). Real gaps, ranked:
  1. Anki lacks deck-state actions: map blacklist→AnkiConnect `suspend`, and a never-forget analog (tag + treat-as-known in status index). `cards/srs-providers.ts`, `cards/action-controller.ts:performApiDeckStateAction`.
