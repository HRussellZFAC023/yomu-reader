...
const HOSTED_DOCS_JA_COPY: Record<string, string> = {
    ...
    'Keep Study audio, CORS fallback, and hosted/newtab behavior on the backlog until the no-userscript, CORS-blocked, and newtab smokes pass together.': 'ユーザースクリプトなし、CORSブロック、newtabの各スモークが同時に通るまで、Study音声、CORSフォールバック、ホスト版/newtab挙動はバックログに残します。',
    'Kept generic mobile article/prose annotations inside the viewport on narrow pages such as Investing.com while preserving furigana on readable body text and leaving compact navigation/footer labels layout-safe.': 'Investing.comのような幅の狭いページで、汎用モバイル記事/本文の注釈をビューポート内に収めつつ、読みやすい本文のふりがなは維持し、コンパクトなナビゲーション/フッターラベルはレイアウト安全なままにしました。',
    ...
    'Aligned review grading shortcuts in Settings > Shortcuts so grade controls start together instead of sharing the row with Study navigation keys.': '設定 > ショートカットの復習採点ショートカットを整列し、採点キーが学習ナビゲーションキーと同じ行に混ざらず一緒に始まるようにしました。',
    ...
    'OCR lets you look up Japanese text inside images. よむ can use embedded OCR metadata when a site provides it, or a local OCR app/server for MangaOCR, PaddleOCR, Apple Vision-style results, and compatible local JSON responses.': 'OCR機能により、画像内の日本語テキストを検索できます。よむは、サイトが提供している埋め込みOCRメタデータを使用するか、MangaOCR、PaddleOCR、Apple Vision形式の結果、互換ローカルJSONレスポンスのためのローカルOCRアプリ/サーバーを使用できます。',
