...
- **A local OCR endpoint.** Point よむ at a local OCR app or server and it can recognize text on demand. It understands **MangaOCR**, **PaddleOCR**, Apple Vision-style wrappers, and compatible local JSON responses, including YomiNinja-style endpoints.

## Good for

- **Manga** — read raw Japanese manga panel by panel, checking only the words you don't know.
- **Screenshots & copied game lines** — use a capture/OCR tool such as YomiNinja, then look up recognized text without retyping.
