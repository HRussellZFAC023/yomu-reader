...
const HOSTED_DOCS_JA_COPY: Record<string, string> = {
    ...
    'Website locale changes keep the document language, direction, accessible navigation labels, canonical URL, Open Graph metadata, hreflang links, and structured breadcrumbs in sync during client-side navigation. Routes without reviewed Japanese copy fall back to the Japanese homepage instead of linking to a translated 404.': 'ウェブサイトの表示言語を切り替えると、クライアント側でページを移動しても、文書の言語と方向、アクセシブルなナビゲーション名、正規URL、Open Graphメタデータ、hreflangリンク、構造化パンくずが同期されます。確認済みの日本語版がないページは、翻訳版の404へリンクせず日本語ホームへ戻ります。',
