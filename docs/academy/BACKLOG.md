...
**Active task:** `day:1:lesson-zero:08` - complete the saved-name card as a canonical story journey rather than an isolated component proof: keep the one name chosen during arrival, help the learner confirm or choose its katakana form without assuming kana, teach only the useful `名前 + です` move, replace generic pronunciation with accepted Rie lines, require a changed-person spoken or recognition transfer before mastery, persist the exact cursor without copying profile data, and prove story reachability, repair, reload, SRS, responsive accessibility and return on the real phone/tablet/desktop path.
