...
**Current delivery checkpoint:** the delivery tree is synchronized through Reader `v1.7.2`; shared story, audio, N3, account lifecycle, workflow, retained upstream work, and the approved Aakash `v009` runtime promotion are consolidated. The executable Day 1 manifest contains 53 learner-facing surfaces and 265 closure proofs. It currently records **150 verified**, **0 partial**, and **115 unverified** dimensions; strict Day 1 closure is **`30/53`**.

**Closed Day 1 journeys:** class-code access, linked-account lifecycle, installed offline resume, learner identity, Rie's first meeting, starting-path choice, reversible manual-level entry, accepted placement, the canonical Academy arrival, Chapter 1, and its ten proved mission activities from the first listening gate through closing the classroom. Their production routes persist exact progress and are covered by automated contracts plus real phone, portrait-tablet, and desktop journeys.

**Latest closure - `day:1:blank-atlas` and the Chapter 1 mission bundle:** all eleven story scenes, nineteen lines at both foundation and N5, choices, lesson handoffs, repairs, living props, music, ambience, SFX, cursor restoration, exact completion evidence, replay and return paths are proved. The nine later mission activities are independently completed from the real story route on phone, portrait tablet and desktop, including audio requests, persistent pass outcomes, serious/critical Axe checks, responsive geometry and clean consoles. The final name-card mission now uses the one saved learner name, helps compare its katakana form without requiring prior kana, and never creates a duplicate naming step.

**Latest closure - `day:1:lesson-zero:02` and optional vowel bingo:** five real carrier words now introduce one sound at a time through exact reviewed Aivis lines, a learner-controlled visual equivalent, post-listen choices, bounded repair, delayed recall, reload-safe progress, an optional nine-tile bingo payoff, and an always-visible route into class. The canonical onboarding journey passes at 320 px, 390 px and 1440 px with clean geometry, serious/critical Axe checks, exact cache-addressed audio playback, and all five accepted lines independently recovered by Whisper and Vosk.

**Latest closure - `day:1:lesson-zero:03` five-vowel writing:** each reviewed carrier word now moves through one look, one independent attempt, one-item repair and delayed mixed retrieval. Drawing and stroke-plan routes produce the same per-kana evidence and SRS identities; a real failed browser stroke earns the guide, a dense canonical trace passes the shared density-invariant grader, and all five exact audio anchors return before completion. Pause, Back, direct resume and reload preserve the exact cursor. The canonical route passes at 320 px, 390 px and 1440 px with stable input geometry, 44 px controls, zero horizontal overflow, clean serious/critical Axe results and a concise route into class.

**Latest closure - `day:1:lesson-zero:04` classroom rhythm:** seven high-frequency movement commands now run as a bounded teach-act-repair-recall loop: one Rie line, three concrete room actions, one-item repair after a lapse, then all seven in a different order. Practice seeds the shared SRS identity; mixed recall records independent evidence without duplicating cards. All seven cache-addressed Aivis lines are Codex-accepted with `humanReviewed: false`, and their exact bytes play through the built route. Save, campus return, direct resume and reload preserve the precise stage. The real onboarding journey passes at 320 px, 390 px and 1440 px with 44 px controls, zero horizontal overflow, clean serious/critical Axe results and no fallback TTS. This closes the seven-command activity only; the complete fourteen-expression classroom set remains open.

**Latest closure - `day:1:lesson-zero:05` repetition request:** Rie now teaches `もう一度お願いします` as two audible, meaningful chunks with no assumed kana, one guided reconstruction, one-item repair and an independent cafe transfer with Aakash. Only the two demonstrated concepts earn mastery; practice creates one canonical SRS review and transfer does not duplicate it. The route reuses the exact reviewed Aivis line without changing its immutable cache identity, queues rapid mobile taps, persists pause/reload/completion, and returns through the first-pass Aakash story seam. The real onboarding journey passes at 320 px, 390 px and 1440 px with focused chrome, 44 px controls, one-line Japanese, no portrait or viewport overlap, clean Axe results and no fallback TTS.

**Latest closure - `day:1:lesson-zero:06` desk language:** Rie now introduces `しゅくだい` and `れい` only beside two visually distinct classroom papers, tells a zero-kana learner to follow sound and prop first, and asks for bounded recognition before swapping the papers and removing the English labels. A deliberate confusion repairs only `しゅくだい`; the two practice passes schedule exactly two stable SRS identities, while changed-layout transfer adds independent evidence without duplicate cards. Both accepted Aivis lines play from their exact cache-addressed Opus assets. Pause, campus return, direct resume, reload, completion evidence, the learner journal and the next Aakash story seam are proved through the real onboarding route at 320 px, 390 px, 1024 px portrait tablet and 1440 px desktop with zero Axe violations, 44 px controls, intact Japanese labels and no fallback TTS.

**Latest closure - `day:1:lesson-zero:07` first sentences:** Rie now introduces five useful sentence shapes through short exchanges before the learner builds them: identity, correction, a question for Sophie, a class link with `の`, and an inclusive reply with `も`. Six pictographs anchor the vocabulary without assuming kana; the answer remains hidden until a committed lapse, repair targets only the failed shape, and the final turn requires all five from memory in changed contexts. Each shape schedules one stable SRS identity, while repair and transfer evidence remain distinct. Fourteen accepted cache-addressed Rie/Sophie lines play through the production route with no generated-TTS fallback. Pause, story return, direct resume, reload, completion evidence, the learner journal and the Aakash handoff are proved from the real onboarding route at 320 px, 390 px, 1024 px portrait tablet and 1440 px desktop with zero serious/critical Axe violations, no horizontal overflow, and responsive character staging. Day 1 delivery-row IDs are now explicit so adding a new activity cannot silently renumber this or any later checkpoint.

**Latest closure - `day:1:lesson-zero:08` saved class name:** the first naming moment now keeps one chosen class name, defaults to its katakana form without assuming kana, and teaches only `名前 + です` through Rie’s accepted line, an ordered build, earned repair, and a changed-person transfer. The session persists only stable choices and cursor state, schedules one SRS identity after transfer, restores through Back and reload, and prevents the later class-card activity from asking for the learner’s name again. The real onboarding journey passes at 320 px, 390 px, 1024 px portrait tablet and 1440 px desktop with no generated-TTS fallback, horizontal overflow, clipped controls, serious/critical Axe violations, duplicate journal evidence, or duplicate naming.

**Latest closure - `day:1:lesson-zero:09` two-name listening:** Xingyu and Mika now introduce themselves in two short exchanges before either name is tested. Transfer changes both speaker and order: Mika introduces Xingyu, then Xingyu introduces Mika. A miss replays only the needed line, a second miss completes with visible support instead of trapping a new learner, and the two stable name identities enter SRS without duplicate cards. The exact activity cursor survives pause, campus return and reload; completion writes one natural class-journal memory and returns to the next story beat. All four cache-addressed Aivis lines play through the production route with no browser-TTS fallback. The real onboarding journey passes at 320 px, 390 px, 1024 px portrait tablet and 1440 px desktop with 44 px controls, natural Japanese wrapping, no horizontal overflow, no lookup-overlay collision, clean serious/critical Axe results and a clean console.

**Active task:** `day:1:lesson-zero:10` - replace the abstract two-gap text probe with a concise Sophie-and-Ruparna library exchange. Reintroduce `の` and `も` in two meaningful, visible lines before recall; let the learner use the note itself as evidence; hide answers until a committed attempt; repair only the missed link; change the surrounding names and meaning for transfer; preserve exact progress; seed only the stable particle identities; and prove the complete story handoff, responsive reading layout, character art, pause/reload and return seam on phone, tablet and desktop.
...
<a id="base-020"></a>
- [ ] **BASE-020** Ship a native-band Aivis learning-voice slice with exact source/model/style/licence locks, archived canonical queries, deterministic Opus cache keys, request-owned cancellation and worker/browser fallback, objective acoustic/ASR QA, explicit Codex acceptance with `humanReviewed: false`, and live production proof. **Deps:** `BASE-002`. **Proof:** `C,R,T,Q,S,O,D`; currently 31 of 33 reviewed candidates are accepted on 33 runtime bindings. Miller's cafe line, Rie's repair, greeting, classroom, desk-language and sentence-frame lines, Xingyu's five beginner vowel anchors, and Sophie's sentence-frame replies are built and route-bound. Every shipped file passes deterministic identity and acoustic checks; representative passing waveform samples cover each pinned voice mapping without claiming line-by-line human review. The narrator numeral mismatch and Mary's polite-morpheme mismatch remain rejected. Deployment, service-worker-controlled offline playback, mobile/WebKit, and production a11y proof remain pending in `learning-voice-production-proof.json`.
<a id="base-021"></a>
- [x] **BASE-021** Correct the Honen Day One draft into a complete-beginner Japanese course, map its 21 topics, 63 activities and 105 flashcards into a typed Yomu augmentation catalogue, and mount one adjacent-vowel repair through Xingyu's existing story, audio, grading, accessibility and SRS flow without an iframe or hosted-course shell. **Deps:** `BASE-003`, `BASE-004`. **Proof:** `C,R,T,Q,S,O`; phone and desktop routes are verified. This is one bounded augmentation slice: eight source imports are verified and 1,969 direct candidates remain pending rather than being claimed complete.
...
<a id="art-003"></a>
- [ ] **ART-003** Produce and approve coherent neutral portraits/sprites for every Academy and textbook speaking role plus four protagonists; correct likeness/style families before expanding poses/expressions, and enforce consent/name-only fallback at runtime. Sam's neutral and listening review pair now use the photo-aligned very close-cropped crew-cut silhouette with minimal crown and side volume, and Tom's pair now uses the warmer shape-simplified illustrated treatment instead of the over-realistic face rendering; the remaining cast families still require the same likeness/style pass. **Deps:** `ART-001`, `CAST-001`. **Proof:** `C,R,T,Q,S,O`.
<a id="art-004"></a>
