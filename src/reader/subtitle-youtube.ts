...
function youtubeCaptionCandidateMatchesTrack(candidate: YouTubeCaptionTrackCandidate, track: YouTubeSubtitleTrack): boolean {
    return youtubeCaptionTrackIdentity(candidate) === youtubeCaptionTrackIdentity(track)
...
function simpleYouTubeCaptionName(record: { name?: { simpleText?: string } }): string {
    return record.name?.simpleText ?? '';
}

function runYouTubeCaptionName(record: { name?: { runs?: Array<{ text?: string }> } }): string {
