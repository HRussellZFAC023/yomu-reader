...
**Active task:** `day:1:lesson-zero:06` - replace the two-item desk-language worksheet with a concise Rie-led notice/handout loop: teach `しゅくだい` and `れい` only when their real classroom props make the distinction useful, keep kana optional at first contact, require independent identification in a changed layout, repair only the confused label, seed exact SRS identities, bind reviewed audio, and prove story reachability, persistence, responsive accessibility and return.
