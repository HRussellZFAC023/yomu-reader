...
const HOSTED_DOCS_JA_COPY: Record<string, string> = {
    ...
    'Fixed': '修正',
    'Aligned the Reader settings furigana state checkboxes with the surrounding control grid instead of splitting them into oversized columns.': 'Reader設定のふりがな状態チェックボックスを周囲のコントロールグリッドに合わせ、幅広すぎる列に分かれないようにしました。',
    'Kept source editor order and remove controls in separate fixed columns so the down-arrow and remove buttons no longer collide in Settings -> Sources.': 'ソース編集行の順序ボタンと削除ボタンを別々の固定列に配置し、Settings -> Sourcesで下矢印と削除ボタンが重ならないようにしました。',
