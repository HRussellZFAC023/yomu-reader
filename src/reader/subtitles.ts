...
export class SubtitlePlayerController {
    ...
    private applyVideoInsetForTranscriptLayout(layout: TranscriptPanelLayout): void {
        ...
        if (layout.placement === 'bottom') {
            this.applyPageVideoInset('bottom', layout.top - videoRect.top - layout.margin, layout.height);
            return;
        }
        const availableWidth = this.availablePlayerWidthForSideLayout(layout, videoRect);
        this.applyPageVideoInset(layout.placement, Math.max(0, availableWidth), layout.width);
    }
    ...
    private applyPageVideoInset(side: SubtitleVideoInsetSide, playerSize: number, panelSize?: number): void {
        if (this.fullscreen) {
            this.clearVideoInsetForTranscriptPanel();
            return;
        }
