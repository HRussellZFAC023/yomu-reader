...
- P1 (2026-06-10): Provider parity (Anki/JPDB/Jiten same abilities; audited 2026-06-10, corrected against passing smokes — Jiten new-tab queue and Jiten stats ARE supported). Real gaps, ranked:
  ...
  1b. Jiten study-page addon mounts enabled in 0.6.56 (`isJitenStudyPage` in jiten-page-targets.ts, widened headword sizes) — NEEDS LIVE VERIFICATION on jiten.moe/srs/study; the test account is currently LOCKED ("too many failed login attempts") — retry after the lockout clears, verify mounts and per-card refresh on grade.
