...
function createPublicCache(storageKey, { ttlMs = DEFAULT_TTL_MS, limit = DEFAULT_LIMIT } = {}) {
  ...
  const entries2 = Object.entries(state);
  if (entries2.length <= limit) return;
  entries2.sort((a, b) => a[1].t - b[1].t).slice(0, entries2.length - limit).forEach(([key]) => delete state[key]);
  }
  return {
...
function publicParseWordFromCard(card) {
  const wordId = finiteInteger$1(card.jitenWordId) ?? finiteInteger$1(card.vid);
  const readingIndex = finiteInteger$1(card.jitenReadingIndex) ?? finiteInteger$1(card.sid);
...
function normalizePublicParseWord(value) {
  ...
  const wordId = finiteInteger$1(value.wordId);
  const readingIndex = finiteInteger$1(value.readingIndex);
...
function pitchPatterns(value, reading) {
  return Array.isArray(value) ? value.map(finiteInteger$1).filter((position) => position !== void 0).map((position) => pitchPatternFromPosition(reading, position)).filter(Boolean) : [];
}
        ...
        <details class="jpdb-reader-local-entry jpdb-reader-dictionary-group jpdb-reader-jpdb-used-in-group jpdb-reader-jiten-related-group" ${sourceAttributes(definitionSourceStateKey(stateKey))}>
            ...
            <div class="jpdb-reader-local-glossary">
                <ul class="jpdb-reader-jpdb-used-in jpdb-reader-jiten-related-words">
                    ${entries2.slice(0, 20).map((entry) => renderJitenRelatedWord(entry, language)).join("")}
                </ul>
            </div>
        </details>
    `;
...
function jitenRelatedWordReading(entry) {
  return jitenAnnotatedKana(entry.readingFurigana) || cleanJitenAnnotatedText(entry.reading);
}
function renderJitenRelatedReference(entry, lookup, reading) {
...
function renderJitenRelatedMeta(entry) {
...
function renderJitenRelatedDefinition(definition) {
...
function renderJitenAudioButton(text, language, extraAttributes = "", command) {
