...
- P1 (2026-06-10): Provider parity (Anki/JPDB/Jiten same abilities; audited 2026-06-10, corrected against passing smokes — Jiten new-tab queue and Jiten stats ARE supported). Real gaps, ranked:
  1. Anki lacks deck-state actions: map blacklist→AnkiConnect `suspend`/`unsuspend` (toggle via `areSuspended`), never-forget→`addTags 'yomu-never-forget'` treated as known in the status index. Route in `cards/action-controller.ts:performApiDeckStateAction` for Anki-backed cards; add invoke wrappers in `anki/index.ts`; gate popover buttons; tests with mocked AnkiConnect.
  ...
  1d. Jiten daily stats cache (heatmaps/streaks): new jiten-stats-cache storing {date, newCardsToday, reviewsToday} snapshots in GM storage on each study-batch load; aggregate into the stats activity view like the Anki source.
...
- P1 (2026-06-10): Instant-colorisation audit remainders (head fixes shipped 0.6.58 — warmup head unpaced, 1s enrichment retry, 48-wide apply chunks, single contrast pass). Remaining root causes with evidence: (1) two-pass coloring — Anki status applied after DOM render causes color pop-in; pre-warm the status cache at scan start and merge state into the initial render (visible-page-scanner.ts:160, main.ts:4804-4820); (2) no persistent IntersectionObserver — fast scrolls parse stale regions first (dom/index.ts collectVisibleTextTargets); (3) cold-start sequential local pitch lookups (lookup/parser.ts:254-276); (4) MutationObserver paused across whole enrichment cycles delays newly inserted text; (5) thundering-herd first Anki status batch — chunk into ~50-item parallel requests.
