...
const HOSTED_DOCS_JA_COPY: Record<string, string> = {
    ...
    'Kept OCR overlay text dark by default in dark mode too, using a light readable highlight instead of falling back to white text.': 'ダークモードでもOCRオーバーレイの文字は標準で暗いままにし、白文字へ戻す代わりに読みやすい明るいハイライトを使うようにしました。',
    ...
    'Added a game text bridge guide that replaces the OCR helper-only workflow, explains the current PWA/no-install boundary, and compares native helper/native OCR helpers for game text handoff.': 'OCR helperだけに絞った従来のワークフローを置き換えるゲームテキストブリッジガイドを追加しました。現在のPWA／インストール不要ルートの境界を説明し、ゲームテキストの受け渡しに使えるnative helper／ネイティブOCRヘルパーを比較しています。',
    ...
    'Read games with OCR helper and よむ': 'OCR helperとよむでゲームを読む',
    'Open the gaming guide': 'ゲームガイドを開く',
    'OCR helper releases': 'OCR helperリリース',
    'This OCR helper-only guide has been replaced by the broader game text bridge guide.': 'OCR helperだけに絞ったこのガイドは、より広いゲームテキストブリッジガイドに置き換わりました。',
    'OCR helper is still useful on desktop: it extracts game or video text, overlays the recognized text, and supports browser dictionary extensions. For Steam Deck-first play, start with the new guide so you can choose between browser-readable text, native helper/native OCR helpers, and desktop mining tools without assuming OCR helper is the only path.': 'OCR helperはデスクトップでは今でも有用です。ゲームや動画のテキストを抽出し、認識したテキストを重ねて表示し、ブラウザ辞書拡張にも対応します。Steam Deckを優先する場合は、OCR helperだけを前提にせず、ブラウザで読めるテキスト、native helper／ネイティブOCR補助ツール、デスクトップのマイニングツールから選べる新しいガイドから始めてください。',
    ...
    'A native helper or native OCR overlay such as OCR helper or native helper Translator': 'OCR helperやnative helper Translatorのようなnative helper／ネイティブOCRオーバーレイ',
    ...
    'OCR helper, GameSentenceMiner, Kamite, Translumo, Kamui, Agent, Textractor, or a local OCR endpoint': 'OCR helper、GameSentenceMiner、Kamite、Translumo、Kamui、Agent、Textractor、またはローカルOCRエンドポイント',
    ...
    'Steam Deck OCR helper': 'Steam Deck OCR helper',
    'Steam Deck-oriented OCR/lookup flow with native helper-style capture behavior.': 'native helper風のキャプチャ動作を持つSteam Deck向けOCR／検索フロー。',
    ...
    'native helper Translator': 'native helper Translator',
    ...
    'Desktop OCR helper': 'Desktop OCR helper',
    ...
    'GameSentenceMiner': 'GameSentenceMiner',
    ...
    'Game text bridge guide': 'Game2Text Steam Deckガイド',
    ...
    'use native helper or a native helper for screen capture, global buttons, and overlays;': '画面キャプチャ、グローバルボタン、オーバーレイにはnative helperまたはネイティブ補助ツールを使います。',
    ...
    'On Steam Deck, expect native helper plugins and native helpers to have broader device access than a normal webpage.': 'Steam Deckでは、native helperプラグインやネイティブ補助ツールが通常のWebページより広いデバイスアクセスを持つ前提で扱ってください。',
    ...
    'You want live lookup while playing: use a native overlay or native helper-style tool for the play session, then mine the good lines with よむ afterward.': 'プレイ中にライブ検索したい：プレイ中はネイティブオーバーレイまたはnative helper風ツールを使い、後で良い行をよむでマイニングしてください。',
    'Open Study': '学習を開く',
    'OCR guide': 'OCRガイド',
    ...
    'That split matters. A browser or PWA is the cleanest path when another tool already exposes the text, but Steam Deck Game Mode capture and overlays still need a native, native helper, or packaged helper.': 'この分担が重要です。別のツールがすでにテキストを公開しているならブラウザやPWAが最もきれいな経路ですが、Steam Deck Game Modeのキャプチャとオーバーレイにはまだネイティブ、native helper、またはパッケージ化された補助ツールが必要です。',
    ...
    'The first Yomu-owned gaming app should be a Gaming Text Bridge rather than a full capture engine: a Steam Deck-friendly companion surface that accepts typed, pasted, OCR, clipboard, texthooker, or helper-app text and presents it as a normal よむ reader surface. Native capture can stay behind a native helper/Electron helper until a prototype proves it can be secure, simple, and maintainable.': 'よむが最初に自前で作るゲーム向けアプリは、完全なキャプチャエンジンではなくGaming Text Bridgeにするべきです。これは入力、貼り付け、OCR、クリップボード、テキストフッカー、補助アプリからのテキストを受け取り、通常のよむ読み取り面として表示するSteam Deck向けの補助面です。安全で単純かつ保守可能だとプロトタイプで示されるまでは、ネイティブキャプチャはnative helper／Electronヘルパーの裏側に置けます。',
    ...
    'A free paid immersion suite alternative': '無料のpaid immersion suite代替',
    ...
    'paid immersion suite comparison': 'paid immersion suiteとの比較',
    ...
    'Where paid immersion suite may suit you better': 'paid immersion suiteのほうが向いている場合',
    'A free alternative to paid immersion suite for Japanese immersion': '日本語没入のためのpaid immersion suiteの無料代替ツール',
    'Looking at': '検討しているのが',
    ...
    'Price': '価格',
    'Free': '無料',
    'Paid subscription — see vendor site': '有料サブスクリプション — vendor siteをご覧ください',
    'Platform': 'プラットフォーム',
    'Browser userscript (desktop + iOS/iPad)': 'ブラウザのユーザースクリプト（デスクトップ + iOS/iPad）',
    'See vendor site': 'vendor siteをご覧ください',
    ...
    'free, no account. paid immersion suite is a separate paid, integrated suite; see vendor site for its features and pricing.': '無料で、アカウントは不要です。paid immersion suite は別の有料の統合スイートです。機能と価格については vendor site をご覧ください。',
    'No, and it does not try to. It focuses on browser reading and mining with your own Jiten, JPDB, Anki and Yomitan dictionaries; paid immersion suite is the all-in-one suite some people prefer.': 'いいえ、そしてそれを目指してもいません。ご自身の Jiten、JPDB、Anki、Yomitan 辞書を使った、ブラウザでの読書とマイニングに重点を置いています。paid immersion suite はオールインワンのスイートで、それを好む人もいます。',
    'Next steps:': '次のステップ：',
    ...
    'Read game dialogue with OCR helper and よむ-compatible study flows.': 'OCR helperとよむ互換の学習フローで、ゲームの会話を読めます。',
    ...
    'Use OCR helper for desktop game OCR, then bring useful text into your study flow.': 'デスクトップゲームのOCRにはOCR helperを使い、役立つテキストを学習フローへ持ち込めます。',
    ...
    'Read games with OCR helper': 'OCR helperでゲームを読む',
    ...
    'Use the first-party desktop app for PC game capture and lookup.': 'PCゲームのキャプチャと検索には、ファーストパーティのデスクトップアプリを使えます。',
    ...
    'OCR lets you look up Japanese text inside images. よむ can use embedded OCR metadata when a site provides it, or a local OCR app/server for MangaOCR, PaddleOCR, Apple Vision-style results, and compatible local JSON responses, including OCR helper-style endpoints.': 'OCR機能により、画像内の日本語テキストを検索できます。よむは、サイトが提供している埋め込みOCRメタデータを使用するか、MangaOCR、PaddleOCR、Apple Vision形式の結果、OCR helper形式のエンドポイントを含む互換ローカルJSONレスポンスのためのローカルOCRアプリ/サーバーを使用できます。',
    ...
    'Jiten': 'Jiten',
    'Tailscale': 'Tailscale',
    'paid immersion suite': 'paid immersion suite',
