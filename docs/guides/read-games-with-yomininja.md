...
For games, keep the tools separate:
...
よむ does not launch YomiNinja, choose your game window, or run game OCR by itself. The handoff is the text: once a useful line is browser-readable, よむ can look it up and mine it like any other sentence.
...
That gives you the line without retyping it.

## 2. Move good lines into a browser surface

When a line is worth studying, put it somewhere よむ can read:

- **A browser note or scratch page.** Copy the OCR text into a browser note, local HTML page, or simple text page.
...
YomiNinja is built with Electron and has partial Chrome-extension support. If you try dictionary tools inside YomiNinja itself, test them on YomiNinja's **Extensions** page before relying on them during a game.
