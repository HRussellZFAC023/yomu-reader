...
**Active task:** `day:1:lesson-zero:05` - turn the repetition request into a concise Rie-led reconstruction loop with no assumed kana: explain why it matters, teach only the needed sound/meaning chunks, require an independent rebuild, repair only the slipped chunk, revisit it through SRS and changed-context transfer, then prove exact audio, persistence, responsive accessibility and the real story/lesson return path.
