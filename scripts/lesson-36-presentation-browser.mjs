...
try {
    for (const viewport of viewports) {
        ...
        await assertAccessible(page, '.academy-authored-week-story-context', `${viewport.name} story presentation`);
        await renderActivity(page);
        await assertActivity(page, viewport);
        await assertAccessible(page, '#lesson-36-browser-host', `${viewport.name} activity`);
        assert.deepEqual(errors, [], `${viewport.name} browser console must stay clean`);
        await context.close();
    }
    console.log('Lesson 36 browser and accessibility checks passed at 1280x900 and 390x844.');
} finally {
    await browser.close();
}
...
async function renderPresentation(page) {
    await page.evaluate(async () => {
        ...
        const entry = story.createLessonStoryRuntime(plan).continuity('l2-l11');
        if (!entry) throw new Error('Lesson 36 continuity is unavailable.');
        const presentation = story.lessonStoryPresentation(entry);
        if (!presentation) throw new Error('Lesson 36 presentation is unavailable.');
...
async function assertPresentation(page, viewport) {
    ...
    assert.match(await context.innerText(), /Angel/u);
    assert.match(await context.innerText(), /Alex/u);
...
async function renderActivity(page) {
    await page.evaluate(async () => {
        const [{ createLessonThirtySixOccasionRouteBeat }, { createAcademyActivityRuntime }] = await Promise.all([
            import('/src/academy/content/lesson-thirty-six-occasion-route.ts'),
            import('/src/academy/minigames/index.ts'),
        ]);
        const host = document.createElement('main');
        host.id = 'lesson-36-browser-host';
        document.body.replaceChildren(host);
        createAcademyActivityRuntime().mount(
            createLessonThirtySixOccasionRouteBeat().activity,
            {
                language: 'en',
...
async function assertActivity(page, viewport) {
    const visual = page.locator('[data-source-visual]');
    assert.equal(await visual.count(), 1);
    assert.equal(await visual.getAttribute('data-source-visual'), '/academy/content/lessons/l2-l11/moodle-new-chapter-23-1-toki-page-1.png');
    ...
    const trigger = visual.locator('.academy-source-visual-trigger');
    const dialog = visual.locator('[data-source-inspector]');
    ...
    await assertAccessible(page, '[data-source-inspector]', `${viewport.name} source inspector`);
