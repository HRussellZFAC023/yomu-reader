...
const SOURCE_ID = `moodle-vocabulary:6974659:${PAYLOAD}`;
const SOURCE_PAGE = 'public/academy/content/lessons/l2-l10/moodle-chapter-23-1-vocabulary-page-1.png';
const SOURCE_PAGE_SHA256 = '4374602cc92bd1a04c8409e3a6a7392fb706f3e038f6afeb9a42aa4ec5ced2f9';

describe('Library SRS l2-l10 exact vocabulary frontier', () => {
    beforeEach(() => localStorage.clear());
    afterEach(() => document.body.replaceChildren());

    it('anchors Moodle, Minna 22-23, and Genki 16 while preserving all 16 source rows', () => {
        const input = lessonPackage();
        const definition = exactLibraryVocabularyDefinition('l2-l10', input);
        const sheet = createLibraryVocabularySheetFromPackage(input, 'l2-l10');
        ...
        expect(crosswalk.lessons.find(lesson => lesson.lessonId === 'l2-l10')).toMatchObject({
        ...
        for (const worker of [
            ...
            expect(worker).toContain('/academy/content/lessons/l2-l10/moodle-chapter-23-1-vocabulary-page-1.png');
        }
    });
    ...
    it('requires the source rows before the existing varied, repairable lesson chapter', async () => {
        ...
        const prerequisite = await loadSenseiVocabularyPrerequisite(
            'authored-week:l2-l10',
            committedAuthoredWeekFetcher(getAuthoredWeekRegistration('l2-l10')),
        );
        ...
        expect(chapter?.beats.map(beat => beat.activity.kind)).toEqual([
            'academy-toki-threshold',
            'academy-minna-true-false-listening',
        ]);
        ...
        expect(chapter?.beats[1]?.activity).toMatchObject({
            provenance: { answerVisibility: 'after-attempt' },
            answerSupport: { earnedHintPolicy: 'explicit-after-attempt' },
        });
    });
    ...
    it('routes only source-glossed rows through Reader, Jiten fallback, and Word to Type', async () => {
        const sheet = createLibraryVocabularySheetFromPackage(lessonPackage(), 'l2-l10');
    ...
    it('returns to station and revisits without duplicating provenance or resetting SRS', async () => {
        const entered = transitionAcademyRoute({
            ...
            kind: 'push',
            route: 'review',
            context: { lessonId: 'authored-week:l2-l10' },
        });
        ...
        const word = libraryStudyVocabulary(
            createLibraryVocabularySheetFromPackage(lessonPackage(), 'l2-l10'),
        )[0]!;
    ...
    it('pins mirrors and ledger claims while rejecting unsupported source mutations', () => {
        expect(sha256File('public/academy/content/lessons/037-l2-l10.json'))
            .toBe('716f456e47d812855f5e5f67a7f704fa93c6a37a2256798588403d36feb28034');
        expect(filesHaveSameContent('docs/public/academy/content/lessons/037-l2-l10.json', 'public/academy/content/lessons/037-l2-l10.json')).toBe(true);
        ...
        expect(ledger.worksheetDigitisation.additionalSlices.find(slice => slice.lessonId === 'l2-l10'))
            .toMatchObject({
                sourcePackage: { sha256: '716f456e47d812855f5e5f67a7f704fa93c6a37a2256798588403d36feb28034' },
                claims: {
        ...
        expect(() => exactLibraryVocabularyDefinition('l2-l10', wrongOwner)).toThrow(/ownership changed/i);

        const wrongWords = structuredClone(lessonPackage());
        mutableRecord(exact(vocabularyRows(wrongWords)[0]!)).words = '聞きます';
        expect(() => exactLibraryVocabularyDefinition('l2-l10', wrongWords))
        ...
        expect(() => exactLibraryVocabularyDefinition('l2-l10', inventedGloss))
        ...
        expect(() => exactLibraryVocabularyDefinition('l2-l10', reframedGloss))
        ...
        expect(() => exactLibraryVocabularyDefinition('l2-l10', wrongPage))
        ...
        expect(() => exactLibraryVocabularyDefinition('l2-l10', relabeledProvenance))
            .toThrow(/field provenance changed at row 14/i);
    });
});

function lessonPackage(): unknown {
    return JSON.parse(readFileSync('public/academy/content/lessons/037-l2-l10.json', 'utf8')) as unknown;
}
