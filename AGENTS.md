...
The audit uses deterministic local network responses for repeatable regression runs. Use `.env` for local-only API keys; it is ignored by Git. This workspace has a local Nadeshiko key stored as `NADESHIKO_API_KEY` in `.env` for live example-source QA. `npm run qa:live` is the narrow live JPDB key smoke test. Any script that needs live JPDB or third-party data must load `.env` through `scripts/lib/qa-env.mjs` before reading env vars.
