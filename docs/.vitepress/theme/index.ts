...
const HOSTED_DOCS_JA_COPY: Record<string, string> = {
    ...
    'Reworked Google Drive settings sync/restore in userscript contexts to use same-tab OAuth redirects instead of popups, with automatic resume after returning from Google.': 'ユーザースクリプト環境のGoogle Drive設定同期/復元を、ポップアップではなく同一タブのOAuthリダイレクトで動くように作り直し、Googleから戻った後に自動で再開するようにしました。',
