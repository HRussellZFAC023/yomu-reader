...
**Current delivery checkpoint:** the delivery tree is synchronized through Reader `v1.6.406`; shared story, audio, N3, account lifecycle, workflow, retained upstream work, and the approved Aakash `v009` runtime promotion are consolidated. The executable Day 1 availability manifest enumerates 52 learner-facing surfaces and 260 closure proofs across implementation, reachability, media, persistence, and full-journey evidence. The first Rie greeting, vowel listening, repeatable listening bingo, five-kana writing, classroom-instruction rehearsal, understanding/repair rehearsal, and living desk-language lesson slice now account for 18 verified dimensions, 17 partial dimensions, and 225 unverified dimensions. No row has all five dimensions verified, so strict Day 1 closure remains `0/52`; partial evidence never counts as a completed activity. The active task is to close every required, optional, and repeatable row in chronological order through the complete welcome-to-first-evening journey.
