...
const COPY = {
    en: {
        ...
        mobileAnkiHandoff: 'Use mobile Anki handoff fallback',
        ...
        ankiUnreachable: 'AnkiConnect is not reachable yet. Troubleshooting: 1. Open Anki on this computer. 2. In Anki, confirm the AnkiConnect add-on is installed and enabled. 3. Press Check AnkiConnect again.',
        ankiHostedCorsHint: 'Hosted page: local Anki access needs the よむ userscript bridge. Enable the userscript on this page, refresh, then press Check again. Advanced fallback: add https://hrussellzfac023.github.io to AnkiConnect\'s webCorsOriginList only if you want direct browser access.',
...
const JA_COPY: Partial<Record<UiCopyKey, string>> = {
    ...
    ankiHostedCorsHint: 'ホスト版ページ: ローカルAnkiへ接続するには、よむユーザースクリプトブリッジが必要です。このページでユーザースクリプトを有効にして再読み込みし、もう一度確認してください。上級者向け: ブラウザから直接接続したい場合だけ、AnkiConnectのwebCorsOriginListに https://hrussellzfac023.github.io を追加してください。',
...
const JA_SETTINGS_COPY: Partial<Record<UiCopyKey, string>> = {
    ...
    mobileAnkiHandoff: 'モバイルAnki受け渡しを代替手段として使う',
    ...
    ankiUnreachable: 'AnkiConnectにまだ到達できません。確認手順: 1. このPCでAnkiを開く。2. Ankiのアドオン画面でAnkiConnectがインストール済み・有効か確認する。3. もう一度「AnkiConnectを確認」を押す。',
