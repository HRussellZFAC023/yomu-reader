...
const UPGRADE_PATHS = [
    ...Object.values(ACADEMY_ASSETS.characterSpriteGalleries.rie),
    ...Object.values(ACADEMY_ASSETS.characterSpriteGalleries.tom2),
    ...Object.values(ACADEMY_ASSETS.characterSpriteGalleries.steve),
] as const;

describe('Academy living-paper character upgrade', () => {
    it('ships three transparent, portrait-scale performances for each upgraded character', () => {
        expect(new Set(UPGRADE_PATHS).size).toBe(9);
    ...
    it('uses distinct left, front, and right character-book slots', () => {
        for (const [character, gallery] of Object.entries({
            ...
            expect(slots, character).toHaveLength(3);
