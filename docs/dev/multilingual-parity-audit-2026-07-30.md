...
**Scope.** 33 registered learning targets (`ja`, `ko`, and 31 `*-roster-v1` generic modules), 18 declared capabilities, measured in `release-worktrees/lens-ocr-scope-20260728` on 2026-07-30. Rows marked **[CORRECTED]** are where an adversarial challenge overturned the original audit verdict. Rows marked **[verified here]** are ones I re-measured myself for this document because two challenges contradicted each other; the measurements are in §7. **Wave 13 correction (2026-07-31):** the real six-target published-archive rerun refuted the lemma-only coverage figures; see `w13-morphology-measurement-2026-07-31.md`.

**Historical status.** Unless a row explicitly carries a later correction,
this document is a 2026-07-30 forensic snapshot rather than current release
documentation. Releases 1.8.51 through 1.8.56 resolved several findings while
leaving the original evidence visible. The changelog is authoritative for
shipped behaviour; the Wave 13 corrections here are intentionally narrow.

---

## 1. The answer at audit time

**No. Full multilingual support does not exist, and most of the capability matrix that is supposed to describe it is read by nothing — so it still neither documents reality nor protects the learner from it.** **Four** of the 18 flags now have a production read site. Wave 13 made `character-lookup` a real runtime gate. `morphology` at `src/reader/languages/morphology.ts:39` still has zero callers; `ocr` at `src/reader/languages/resolve.ts:42` only scopes a settings migration; and `term-lookup` at `config/docs/product-claims.ts:34` is build-time and drives the public homepage. Fourteen flags have no production read site. What *does* exist, and is genuinely good, is a **target-neutral plumbing layer**: ICU segmentation for all 33, OCR for all 33, Tatoeba example sentences for all 32 non-Japanese targets with honest visible degradation, per-target lookup hotlinks for all 32, a language-agnostic SRS store and SM-2 scheduler, and — refuting the prior everyone was working from — **1,637 published dictionary entries covering all 32 roster languages** (34 headword languages; the "zero non-CJK entries" measurement is stale). **Wave 13 correction:** Japanese has the mature deinflector, while Arabic, German, Spanish, and Russian declare bounded rewrites and Korean has bounded particle subsegments; the real six-target archives scored 80.3–100.0%, refuting the constructed lemma-only collapse. What still does not exist is a complete morphology layer for every target, a recommendation path to the non-Japanese dictionaries (`DICTIONARY_CATALOG_TARGET_LANGUAGE = 'ja'` remains a type-level literal), or a target choice at install (`app/onboarding.ts:160` remains a read-only Japanese output). Worse than any absence, three features are **on by default and actively hostile** to a non-Japanese learner: the YouTube immersion filter hides their target-language videos as `non-japanese`, `preferJapaneseSiteLanguage` spoofs navigator language, timezone and geolocation to Japan on every site, and text-to-speech overrides a correct `utterance.lang` with a Japanese voice whenever one is installed. **Actionable summary: Yomu today is a Japanese learning system plus a competent 32-language reading-and-lookup tool with a Japanese skin. That is a real, shippable product — but it is not what the homepage says, and the target picker offers 33 languages with no readiness signal while the interface-locale picker 20 lines below it disables 31 locales with a stated reason.**
...
|---|---|---|---|---|---|
| 1 | **term-lookup** | all 33 | all 33 measured; 32 have at least one exact hit, while `yue` is 0/47 | **[CORRECTED] Target-neutral, with coverage now ratcheted.** The six-target real-archive rerun measured th 80.3%, ru 100.0%, ar 83.3%, ko 93.8%, de 100.0%, es 100.0%; the older lemma-only ratios were not a product baseline | Definitions still depend on published dictionary supply and exact target policy. The 33-target exact-span baseline has 29 targets at or above the proposed 60% bar and four below it. See `w13-morphology-measurement-2026-07-31.md` |
| 2 | **character-lookup** | ja only | ja only | **[CORRECTED] The capability now gates rendering and execution.** A single `Unified_Ideograph` predicate includes supplementary-plane kanji without admitting radicals or iteration marks | zh/yue no longer render a chip or reach JPDB, Jiten, Heisig, RTK, KanjiVG, or local Japanese-labelled character rows. Japanese 𠮟/𩸽 and imported Ext-B rows remain codepoint-safe |
| 3 | **segmentation** | all 33 | all 33 mechanically; lookup compensates for Han boundaries | **[CORRECTED] Boundary quality is explicit target policy.** Han targets use contiguous ideograph runs and exact left-to-right maximal matching instead of trusting ICU word guesses | ICU may still produce 我去/他是/很好 for display segmentation, but lookup searches installed exact expressions and emits nothing when no exact expression exists |
| 4 | **morphology** | ja, ar, de, es, ru | ja mature; four bounded rewrite sets; ko bounded particle subsegments | **[CORRECTED] Partial and honestly measured.** The six real archives score 80.3–100.0% without Yomitan's GPL transformer | The all-target ratchet records outcome coverage rather than inferring quality from this flag |
| 5 | **reading-annotation** | ja only | **ja, zh, yue, ko-hanja** — Han surfaces | **[CORRECTED] [verified here] Works beyond declared, unconfigurably.** Parser supplies explicit rubies for *any* target (`lookup/parser.ts` `localTokenFromMatch`); `sourceTokenRubies` returns them **before** its kana gate; `kanjiRubyParts` then emits a whole-base ruby for a non-kana reading over a Han base. The shared Japanese-script seam now includes assigned supplementary-plane kanji without splitting UTF-16 pairs | zh/yue/ko learners get pinyin/jyutping/hangul ruby painted on the page — while the entire furigana control block is detached as `jp-only` (`settings/form.ts:1152`), so they cannot configure or switch it off. The 29 non-Han targets get nothing, and the popup drops the reading silently |
...
**Score.** Overstating flags (declared true, does not work): **text-to-speech, typing**, and partially **term-lookup** and **subtitles**. Understating flags (declared false, ships anyway): **examples (32), srs (32), ocr (31), grading-buttons (32), reading-annotation (3), frequency (2), handwriting (2), grammar (2, wrongly)**. Roughly honest: **character-lookup, morphology, pronunciation, audio, mining**. The matrix is wrong in **both** directions; `character-lookup` is now enforced, while fourteen flags still have no production read site.
...
| **b3** | **[CORRECTED] Japanese has mature deinflection; ar/de/es/ru have bounded rewrites; ko has bounded particle subsegments. The claimed six-target collapse does not reproduce against real WTY archives** | The 2026-07-31 rerun is 80.3–100.0% per target. Morphology remains an explicit future quality dimension, not a reproduced release blocker | `w13-morphology-measurement-2026-07-31.md` |
| **b4** | **[CORRECTED] ICU Han boundaries are structurally wrong; current WTY evidence produced silence rather than the claimed wrong glosses. Wave 13 landed an exact left-to-right maximal sweep** | Installed exact terms are reachable; reading-index guesses and non-exact expressions are rejected, so silence wins over misinformation | `languages/han.ts`; `dictionaries/yomitan/index.ts` |
| **b5** | **[CORRECTED] Query and import normalization now preserve Thai/Lao SARA AM rather than applying destructive NFKC** | Words such as ทำ/คำ/น้ำ and Lao ຳ forms remain in the spelling the published dictionary indexes | `languages/lookup-normalization.ts`; `dictionaries/yomitan/zip-normalize.ts` |
...
| **b12** | **[CORRECTED] Query and import matching now share target-aware case folding.** The all-target exact-span harness locks the resulting hit and miss set to pinned published dictionaries | Sentence-initial Latin and Cyrillic words and capitalised German nouns are reachable without changing Japanese or case-sensitive script behaviour | `languages/lookup-normalization.ts`; `dictionaries/yomitan/zip-normalize.ts`; `multilingual-parity-ratchet.test.ts` |
...
| c8 | **Term dictionaries — NOT a gap** | The "zero non-CJK entries" prior is **refuted**. **[verified here]**: 1,637 entries, 34 headword languages, all 32 roster covered (`terms:1075`, `pronunciation:478`), revision `2026-07-23.574961e8.wty-95a9151c1beb`. Supply is solved; **recommendation reachability** (b2) and the measured target-specific misses remain |
...
3. **Lookup reaches ordinary running text in the claimed language.** Case folding, Thai/Lao SARA-AM preservation, bounded rewrites for the reproduced families, and an exact maximal Han sweep are now in production. The published-archive harness measures the proposed acceptance bar against ten sentences for every target: 29/33 currently meet ≥60%, while `grc`, `yue`, `lo`, and `mn` remain below it. The compact release replay preserves the authoritative exact hit and miss set; it does not turn that bar into a passing claim.
...
|---|---|
| Does ruby paint for zh/yue/ko? | **Yes.** `sourceTokenRubies` returns explicit `token.rubies` **before** its kana gate; the parser supplies them for any target; `kanjiRubyParts` emits a whole-base ruby for a non-kana reading over a Han base, including assigned supplementary-plane kanji. The "blocked by one kana regex / no producer supplies rubies" account is wrong |
| Does NFKC break Thai/Lao? | **It did; corrected before Wave 13.** U+0E33/U+0EB3 decomposition was real, but `normalizeGenericLookupText` and matching import normalization now preserve SARA AM. The real Thai archive scores 49/61 (80.3%) |
...
| Does the capability matrix have runtime reads? | **Four total in production.** `character-lookup` now gates Japanese-only rendering and execution. `morphology.ts:39` has no callers, `resolve.ts:42` reads `ocr` for a settings migration, and `product-claims.ts:34` reads `term-lookup` at build time. The `sources/examples/*` hits are a different type (`ExampleSourceCapabilities`) |
...
1. **Are the 478 IPA "pronunciation" dictionaries `term_meta` banks with `mode: 'ipa'`, or `terms` banks whose glossary is the IPA string?** Decides whether c3 is a one-mode reader (cheap) or a schema problem. No audit opened an archive. *Settling measurement:* download one `(IPA)` zip and print the first row of each bank file plus `index.json`.
2. **Whether the declared non-Japanese dictionaries actually install, index and return definitions in the running reader.** Every measurement so far used hand-built or probe-imported stores. *Settling measurement:* in a real browser, install `[ES-EN] Wiktionary` from the browse shelf under target `es` and confirm a definition renders on a live Spanish page.
3. **Does Google Translate actually serve `yue`, `lo`, `km`, `mn`, `fil`?** `translation/google.ts:59-84` returns `supported: false` for `grc` only and is optimistic for everything else, with no whitelist. *Settling measurement:* one live request per code with the shipped `tl=` parameter.
4. **Live OCR recognition quality per language against the real Lens endpoint.** All measurements were of the request and the response-filter path, never of accuracy. *Settling measurement:* one real screenshot per script family through the shipped provider.
...
**Verified by me for this document** (so you know what is first-hand): the former NFKC decomposition defect and its corrected SARA-AM-preserving query/import path; the 33 pinned published term archives and their exact-span baseline; the full ruby chain (`localTokenFromMatch` → `sourceTokenRubies` → `kanjiOnlyRubySegments` → `kanjiRubyParts`) and the supplementary-plane-safe Japanese-script seam; the grammar-rule Han matches against 4 natural Chinese/Cantonese sentences plus `JAPANESE_TEXT_RE`; the exhaustive `.capabilities` production sweep; catalogue totals, categories, 34 headword languages and frequency-by-language; interface locales 33/2, lookup-links 32, `LEARNER_LANGUAGES` 32 with no `my`; the `pinnedStudyPlanInputs` pin and its comment; `recallSentenceFromCard`'s override-first order; `studySentenceTiers` and the kana/Han filter on the dictionary tier; the repo's own `vi.waitFor` cloze test; `onboarding.ts:160`'s hardcoded `'日本語 — Japanese'`; `textToSpeechJapaneseVoices`; the unconditional `convertRomajiToKana` at `controller.ts:6630`; the six `jp-only` sites in `settings/form.ts` and `languageFamilyIncludes`; the target picker's absence of a disabled state; `docs/index.md:14`; `docs/faq.md:120`.

**Not verified anywhere, by anyone:**

- **End-to-end installed-artifact archive parity in a real browser.** The Wave 13 harness imports 32 pinned real archives through `YomitanDictionaryStore` under `vite-node` with `fake-indexeddb`. Finnish is too large for a full fake-indexeddb import: its authoritative path scans every archive row, retains the complete production-candidate-reachable subset in archive order, and imports that subset through the same store and matcher. Neither path is the shipped userscript in Chrome or Firefox.
