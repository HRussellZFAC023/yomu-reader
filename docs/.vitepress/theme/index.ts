...
const HOSTED_DOCS_JA_COPY: Record<string, string> = {
    'Nightly browser checks now wait for the actual visible popup and selected Study mode, preserve their full timeout budgets while collecting diagnostics, and tolerate expected startup variance without hiding real assertion failures.': '夜間ブラウザーチェックは、実際に表示されているポップアップと選択済みの学習モードを待つようになりました。診断情報を収集している間もタイムアウト時間をすべて確保し、実際のアサーション失敗を隠すことなく、想定内の起動時間のばらつきに対応します。',
    'Automated userscript builds regenerate the Academy shell after the Reader and companion assets have their final names, so its cache revision always identifies the bytes that will be deployed.': 'ユーザースクリプトの自動ビルドは、Readerとコンパニオンアセットの最終ファイル名が確定した後にAcademyシェルを再生成するようになり、キャッシュリビジョンが常に実際にデプロイされるバイト列を示します。',
    'YouTube’s own controls auto-hide normally again. よむ no longer treats simulated fullscreen as browser fullscreen or reparents its UI into YouTube’s player, so it does not keep the player focused or override the page’s native focus lifecycle.': 'YouTube本来のコントロールが、再び通常どおり自動的に隠れるようになりました。よむは疑似フルスクリーンをブラウザーのフルスクリーンとして扱わず、UIをYouTubeのプレーヤー内へ移動しないため、プレーヤーにフォーカスを残したり、ページ本来のフォーカス処理を上書きしたりしません。',
    'Share, Remix, and the other native Shorts controls remain visible and clickable. よむ no longer places reading-annotation portals over YouTube-owned control surfaces.': '共有、リミックスなど、Shorts本来のコントロールが表示されたまま操作できるようになりました。よむは、YouTubeが所有するコントロール面の上に読み注釈用ポータルを配置しません。',
    'A subtitle cue is now visually immutable once it appears: late dictionary, furigana, pitch, or study-state results are cached for the next cue instead of changing words already on screen.': '字幕キューは、いったん表示された後は見た目が変わらなくなりました。遅れて届いた辞書、ふりがな、ピッチ、学習状態の結果は、画面上の単語を後から変更せず、次のキュー用にキャッシュされます。',
    'Selecting and copying a YouTube page no longer includes よむ’s fixed subtitle overlay or its presentation metadata. Paused subtitle text remains explicitly copyable where that mode is enabled.': 'YouTubeページを選択してコピーしても、よむの固定字幕オーバーレイや表示用メタデータが含まれなくなりました。停止中の字幕テキストは、そのモードを有効にしている場合、引き続き明示的にコピーできます。',
    'Fast pointer movement cannot open a stale earlier word after an asynchronous OCR lookup completes; the current pointer position and target-owned portal geometry win before global overlay hit-testing.': 'ポインターを素早く動かした後に非同期OCR検索が完了しても、以前の単語が誤って開くことはなくなりました。オーバーレイ全体のヒットテストより先に、現在のポインター位置と対象自身のポータル形状が優先されます。',
    'WebKit navigation and tab teardown no longer lets a pending Reader startup dereference a removed document root, eliminating an unhandled lifecycle error during rapid page changes.': 'WebKitでページ移動やタブ終了を行った際、保留中のReader起動処理が削除済みの文書ルートを参照しなくなり、素早いページ切り替え時に発生していた未処理のライフサイクルエラーを解消しました。',
    'Dynamic YouTube and document annotations reuse their projected fragment nodes and skip unchanged geometry and style writes. Fractional browser geometry is serialized consistently across Chromium and WebKit, removing redundant hot-loop CSS work without changing annotation placement.': '動的なYouTubeおよび文書の注釈は、投影用フラグメントノードを再利用し、変化していない形状やスタイルの書き込みを省略するようになりました。小数を含むブラウザー形状はChromiumとWebKitで一貫して直列化され、注釈の位置を変えずに高頻度ループの不要なCSS処理を削減します。',
    'The YouTube performance profiler now replays one fixed workload separately for normal timing, CPU sampling, and exact function-call coverage, and scopes every result to the immutable content-addressed userscript graph. Failed runs retain diagnostics instead of producing an empty or misleading report.': 'YouTubeパフォーマンスプロファイラーは、同じ固定ワークロードを通常の時間計測、CPUサンプリング、正確な関数呼び出しカバレッジで個別に再実行し、すべての結果を不変のコンテンツアドレス付きユーザースクリプトグラフに限定します。失敗した実行でも、空または誤解を招くレポートではなく診断情報を保持します。',
    'The Chrome and Edge extensions start reliably on YouTube again. Chromium can expose a null custom-element registry inside an extension content script; よむ now treats that as an unavailable browser capability instead of aborting before the puck appears.': 'ChromeおよびEdgeの拡張機能が、YouTubeで再び確実に起動するようになりました。Chromiumでは、拡張機能のコンテンツスクリプト内でカスタム要素レジストリがnullになる場合があります。よむは、パックが表示される前に起動を中断するのではなく、そのブラウザー機能を利用できないものとして扱います。',
    'Turning off “Prefer Japanese site language” no longer lets an old per-site cache navigate between Japanese and default-language URLs during startup. Only an explicit settings change may roll back the current URL, and turning the preference back on in the same tab still works.': '「サイトの言語と地域を日本優先にする」をオフにしたとき、古いサイト別キャッシュによって起動中に日本語URLと既定言語URLの間を行き来することがなくなりました。現在のURLを元に戻せるのは設定を明示的に変更したときだけで、同じタブで再びオンにしても正しく動作します。',
    'Completed dictionary and enrichment lookups cancel their fallback timers immediately instead of leaving callbacks alive for up to 6.5 seconds and later logging false timeout messages, reducing needless background wakeups during repeated lookups.': '辞書と補足情報の検索が完了した時点で、フォールバック用タイマーを直ちに取り消すようになりました。最大6.5秒間コールバックを残したり、後から誤ったタイムアウトログを出したりしないため、検索を繰り返した際の不要なバックグラウンド起動を減らします。',
    ...
    'See the checked local grammar construction and source for every learning target.': 'すべての学習対象について、確認済みの端末内文法構文と出典を確認できます。',
    ...
    'The Grammar card always stays visible after a check. A local match shows the': '文法カードは確認後も表示されたままです。端末内で一致した場合は、',
    'pattern and its target-owned level. Detection is deliberately narrow: a match': 'パターンと、その学習対象が定めるレベルを表示します。検出範囲は意図的に狭く、一致は',
    'is a prompt to inspect the sentence, not a complete parse.': '文を詳しく調べるための手がかりであり、完全な構文解析ではありません。',
    'Every learning target now owns at least one source-checked local construction.': 'すべての学習対象に、出典を確認した端末内構文が少なくとも1件あります。',
    'Japanese keeps its established 307-rule JLPT inventory. The other targets use': '日本語は既存のJLPT別307ルールを維持します。その他の学習対象で',
    'published levels only when the cited curriculum assigns one: CEFR for five': '公開レベルを使うのは、引用したカリキュラムがレベルを付与している場合だけです。5言語ではCEFR、',
    'targets, the source\'s combined A1–A2 band for Turkish, and the HSK Standard': 'トルコ語では出典のA1〜A2統合帯、中国語ではHSK標準',
    'Course scale for Chinese.': '教程の尺度を使用します。',
    'means only “this target\'s first': 'は「この学習対象で最初に確認した',
    'checked local rule”; it is not a proficiency level and is not equivalent across': '端末内ルール」という意味に限られます。習熟度レベルではなく、',
    'languages.': '言語間で同等でもありません。',
    ...
    'Curated local rules: 33 targets, including Japanese': '精査済み端末内ルール：日本語を含む33言語',
    'Reference only: 0 targets': 'リファレンスのみ：0言語',
    ...
    'Local inventory': '端末内ルール一覧',
    'Checked source and bounded surface': '確認済み出典と限定した検出面',
    '307 rules, JLPT': '307ルール、JLPT',
    '; the established detector and regression corpus remain unchanged.': '。既存の検出器と回帰コーパスは変更せず維持しています。',
    '1 rule, Foundation': '1ルール、基礎',
    "Ca' Foscari University: Balkan existential constructions": 'カ・フォスカリ大学：バルカン諸語の存在構文',
    '; Standard Albanian existential': '。標準アルバニア語の存在構文',
    'University of Texas: Classical Greek Online': 'テキサス大学：Classical Greek Online',
    '; Classical Greek negative particle': '。古典ギリシャ語の否定小辞',
    ', with word boundaries that reject compounds such as': '。単語境界により、次のような複合語は除外します：',
    'Michigan State University: Elementary Arabic II': 'ミシガン州立大学：初級アラビア語II',
    '; Modern Standard Arabic nominal negation with forms of': '。現代標準アラビア語で、次の活用形による名詞文の否定：',
    'Chinese University of Hong Kong: Cantonese negation': '香港中文大学：広東語の否定',
    '; copular negation with': '。次を使うコピュラ否定：',
    '1 rule, HSK 3': '1ルール、HSK 3級',
    'Authorised HSK Standard Course Level 3': '公式HSK標準教程3級',
    'Princeton University: 越来越': 'プリンストン大学：越来越',
    '; bounded simplified/traditional forms of': '。簡体字・繁体字の次の形に検出範囲を限定：',
    'Den Danske Ordbog: der': 'デンマーク語辞典：der',
    '; presentative': '。提示構文',
    'with a checked indefinite complement.': 'を、確認済みの不定補部とともに検出します。',
    'Onze Taal: presentative er': 'Onze Taal：提示構文 er',
    '1 rule, CEFR A1': '1ルール、CEFR A1',
    'British Council: using there is and there are': 'ブリティッシュ・カウンシル：there is / there are の用法',
    '; existence with an explicit indefinite or quantity.': '。不定表現または数量を明示した存在構文です。',
    'Kielitoimiston ohjepankki: omistuslause': 'フィンランド言語局ガイド：所有文',
    '; pronominal adessive possessor plus': '。代名詞の接格所有者と次を組み合わせます：',
    '8 rules, CEFR A1': '8ルール、CEFR A1',
    '; eight bounded constructions from its A1 inventory and examples.': '。A1項目一覧と例文から範囲を限定した8構文です。',
    '7 rules, CEFR A1': '7ルール、CEFR A1',
    'Goethe A1 inventory': 'ゲーテA1項目一覧',
    '; seven bounded A1 constructions.': '。範囲を限定したA1の7構文です。',
    'Centre for the Greek Language: mood and negation': 'ギリシャ語センター：法と否定',
    '; Modern Greek indicative negation with': '。現代ギリシャ語で、次を使う直説法の否定：',
    'Hungarian Grammar': 'ハンガリー語文法',
    '; pronominal dative possessor plus': '。代名詞の与格所有者と次を組み合わせます：',
    'Northern Illinois University: sentences and phrases': '北イリノイ大学：文と句',
    '; negative existence with': '。次を使う否定存在構文：',
    'Treccani: ci': 'Treccani：ci',
    'Northern Illinois University: Khmer negation': '北イリノイ大学：クメール語の否定',
    '; punctuation-bounded': '。句読点で範囲を限定した',
    'National Institute of Korean Language: -고 싶다': '韓国国立国語院：願望構文',
    '; desire endings headed by': '。次で始まる願望語尾：',
    'Northern Illinois University: Lao negation': '北イリノイ大学：ラオ語の否定',
    'before a small checked set of verbs and adjectives.': 'を、確認済みの少数の動詞・形容詞の前で検出します。',
    'Utah State University: sum, esse': 'ユタ州立大学：sum, esse',
    '; Classical Latin negative copula': '。古典ラテン語の否定コピュラ',
    'Huree University: An Elementary Mongolian Grammar': 'フレー大学：初級モンゴル語文法',
    '; modern Cyrillic Khalkha nominal negation with': '。現代キリル文字ハルハ語で、次を使う名詞文の否定：',
    'University of Texas: Persian long copulas': 'テキサス大学：ペルシア語の長形コピュラ',
    '; Iranian Persian negative long-copula forms headed by': '。イラン・ペルシア語で、次で始まる長形コピュラの否定形：',
    'Polish Ministry of Education platform: noun and adjective inflection': 'ポーランド教育省プラットフォーム：名詞と形容詞の屈折',
    '; absence or non-possession with': '。次を使う不在または非所有：',
    'Ciberdúvidas: existential haver': 'Ciberdúvidas：存在動詞 haver',
    '; impersonal': '。非人称の',
    'with checked non-temporal complements shared by Brazilian and European Portuguese.': 'を、ブラジル・ポルトガル語と欧州ポルトガル語に共通する確認済みの非時間補部とともに検出します。',
    'Duke University: Romanian verbs': 'デューク大学：ルーマニア語の動詞',
    '; necessity with': '。次を使う必要表現：',
    '; eight bounded A1 constructions.': '。範囲を限定したA1の8構文です。',
    'University of Texas COERLL: Croatian genitive with imati/nemati': 'テキサス大学COERLL：imati/nematiを使うクロアチア語属格',
    '; Shtokavian Croatian absence or non-possession with': '。シュト方言クロアチア語で、次を使う不在または非所有：',
    'and checked genitive forms.': 'と、確認済みの属格形です。',
    '8 rules, CEFR A1-A2': '8ルール、CEFR A1〜A2',
    '; eight bounded constructions from the published inventory.': '。公開項目一覧から範囲を限定した8構文です。',
    'Swedish Academy Grammar': 'スウェーデン・アカデミー文法',
    'Northern Illinois University: Tagalog existential sentences': '北イリノイ大学：タガログ語の存在文',
    '; Tagalog': '。タガログ語の',
    'existential forms.': '存在形です。',
    'Northern Illinois University: Beginning Thai grammar notes': '北イリノイ大学：初級タイ語文法ノート',
    '1 rule, source band A1–A2': '1ルール、出典区分A1〜A2',
    'Yunus Emre Institute: Hayatın İçinden Türkçe A1–A2': 'ユヌス・エムレ・インスティトゥート：Hayatın İçinden Türkçe A1〜A2',
    'and its': 'および、その',
    '`var / yok` lesson': '「var / yok」の課',
    '; existence, possession, and their absence.': '。存在、所有、およびそれらの欠如を扱います。',
    'Northern Illinois University: Lesson 8 grammar': '北イリノイ大学：第8課の文法',
    '; punctuation-bounded completion with': '。句読点で範囲を限定した、次による完了表現：',
    'Yomu bundles only the small detector patterns above. The linked sources define': 'よむが同梱するのは、上記の小規模な検出パターンだけです。リンク先の出典が、',
    'the construction names, scopes, and any published levels; they are not copied': '構文名、範囲、公開レベルを定義します。出典の内容を',
    'into the bundle. A broader unreviewed regex would create false grammar teaching,': '同梱してはいません。精査していない広範な正規表現は誤った文法指導を生むため、',
    'so new coverage grows through checked examples and adversarial near-negatives.': '新しい対応は、確認済み例文と紛らわしい反例を通して拡張します。',
    ...
    'because it is the deepest, not because it is the only one that works: it adds mature deinflection, pitch accent, kanji cards, stroke feedback, recorded audio, and 307 grammar points. The other targets have much narrower target/data depth; only Arabic, German, Korean, Russian, and Spanish currently add bounded morphology beyond literal dictionary-form lookup. Your recommended starter follows the selected target and definition language; for English plus Spanish, that means Spanish terms with English definitions and Spanish IPA in the pronunciation row where Japanese shows pitch accent. The interface itself still speaks only English and 日本語.': 'と表示されますが、これは「唯一使える言語」という意味ではなく「もっとも深い」という意味です。成熟した活用形解析、ピッチアクセント、漢字カード、筆順フィードバック、録音音声、307の文法項目があります。その他の学習対象では、対象言語やデータによる深さが大きく限られ、辞書形そのままの検索を超える限定的な形態処理があるのは、現在アラビア語、ドイツ語、韓国語、ロシア語、スペイン語だけです。おすすめの初期辞書は、選んだ学習対象と言語定義の言語に合わせて変わります。英語とスペイン語なら、スペイン語の単語、英語の語義、そして日本語のピッチアクセントと同じ発音欄にスペイン語のIPAが表示されます。インターフェースは現在も英語と日本語だけです。',
