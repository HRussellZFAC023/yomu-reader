...
5. An agent response never closes work. Gate evidence must be a `yomu-academy.gate-attestation/v1` JSON document naming the task, gate, exact HEAD, passing verdict, producer, summary, and hash-bound assertion artifacts. Test commands must be executed by `run-proof`, which records their exit code, transcript and exact Git HEAD. Independent review must be a `yomu-academy.review-attestation/v1` document with a `ship` verdict, task-definition hash, exact HEAD, provider/model/session identity, explicit scope, and no unresolved P0/P1 finding. An opaque file or a review that says `BLOCK` cannot be attested as passing.
