...
/** Add the one machine-translated target option an English-only track list needs. */
export function ensureTranslatedTargetTrack(
    ...
    tracks.push({
        ...
        kind: source.kind,
        language: targetSubtitleLanguageTag(),
        autoGenerated: true,
...
export function autoSelectablePageTrackRole(
    ...
    if (isTargetLanguageSubtitleTrack(option)
...
export function findAutoPrimaryYouTubeTrack(
    ...
    const candidate = [...tracks]
        .filter(track => track.kind === 'youtube' && isTargetLanguageSubtitleTrack(track))
...
function inferHostedSubtitleFileJobs(files: File[]): HostedSubtitleFileJob[] {
    ...
    const primary = subtitleFiles.filter(file => isTargetLanguageSubtitleTrack({ label: file.name, kind: 'file' }));
