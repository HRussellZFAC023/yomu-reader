...
import type {
    KatakanaRowSwitchboardModel,
    KatakanaRowSwitchboardResponse,
    KatakanaSwitchboardRowId,
    KatakanaSwitchboardVowelId,
} from './manifest';

export function gradeKatakanaRowSwitchboard(
    model: KatakanaRowSwitchboardModel,
    response: KatakanaRowSwitchboardResponse,
): GradeResult {
    ...
    const missed = model.payload.rounds.filter(round => {
        ...
        return !answer || answer.rowId !== round.rowId || answer.vowelColumnId !== round.vowelColumnId;
    });
...
export function katakanaRowSwitchboardReviewSeeds(
    model: KatakanaRowSwitchboardModel,
    result: GradeResult,
): readonly ReviewSeed[] {
...
export function validateKatakanaRowSwitchboard(model: KatakanaRowSwitchboardModel): readonly ValidationIssue[] {
    const issues: ValidationIssue[] = [];
    if (!model.answerSupport) issues.push({ path: 'answerSupport', message: 'The assessed switchboard needs answer support.' });
    ...
    if (!Array.isArray(rounds) || rounds.length !== 10) {
        issues.push({ path: 'payload.rounds', message: 'The row switchboard requires ten katakana signals.' });
    } else {
        ...
        for (const [index, round] of rounds.entries()) {
            ...
            unique(coordinate(round.rowId, round.vowelColumnId), `${path}.coordinate`, coordinates, issues);
            if (!['na', 'ha'].includes(round.rowId)) issues.push({ path: `${path}.rowId`, message: 'Every signal must identify the na or ha source row.' });
            ...
            if (!model.conceptIds.includes(round.conceptId)) issues.push({ path: `${path}.conceptId`, message: 'Every switchboard concept must belong to the activity.' });
        }
        if (coordinates.size !== 10) issues.push({ path: 'payload.rounds', message: 'Every na/ha and vowel coordinate must occur once.' });
        if (concepts.size !== model.conceptIds.length || model.conceptIds.some(id => !concepts.has(id))) {
            issues.push({ path: 'conceptIds', message: 'Switchboard concepts must exactly match its ten signals.' });
        }
    }
    ...
    if (support?.minna?.reference !== 'Minna no Nihongo I, Katakana strand'
        ...
        || support.genki?.taskId !== 'genki-2e:l1-l25:lesson-2-literacy-wb-5'
    ...
    if (!localized(model.payload?.switchboardLabel)) issues.push({ path: 'payload.switchboardLabel', message: 'A bilingual switchboard label is required.' });
    if (!localized(model.payload?.feedback?.pass?.explanation)
        ...
        issues.push({ path: 'payload.feedback', message: 'The switchboard needs bilingual feedback and a repair ladder.' });
    }
    return issues;
}
...
function validateResponse(
    model: KatakanaRowSwitchboardModel,
    response: KatakanaRowSwitchboardResponse,
): ReadonlyMap<string, Readonly<{ rowId: KatakanaSwitchboardRowId; vowelColumnId: KatakanaSwitchboardVowelId }>> {
    if (!response || !Array.isArray(response.answers) || response.answers.length !== model.payload.rounds.length) {
        throw new TypeError('Katakana row switchboard requires one row and vowel setting for every signal.');
    }
    const signalIds = new Set(model.payload.rounds.map(round => round.id));
    const coordinates = new Set(model.payload.rounds.map(round => coordinate(round.rowId, round.vowelColumnId)));
    const answers = new Map<string, Readonly<{ rowId: KatakanaSwitchboardRowId; vowelColumnId: KatakanaSwitchboardVowelId }>>();
    for (const answer of response.answers) {
        const setting = answer && coordinate(answer.rowId, answer.vowelColumnId);
        if (!answer || !signalIds.has(answer.signalId) || !coordinates.has(setting) || answers.has(answer.signalId)) {
            throw new TypeError('Katakana row switchboard has an unknown or duplicate signal setting.');
        }
        answers.set(answer.signalId, { rowId: answer.rowId, vowelColumnId: answer.vowelColumnId });
    }
    return answers;
}

function coordinate(rowId: string, vowelColumnId: string): string { return `${rowId}:${vowelColumnId}`; }
