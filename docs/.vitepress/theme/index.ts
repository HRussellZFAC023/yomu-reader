...
const HOSTED_DOCS_JA_COPY: Record<string, string> = {
    ...
    'Bunpro pronunciation recordings are available as a new opt-in audio source. It is added disabled to both new and existing audio-source lists, so no Bunpro audio request is made until the user enables it. Recordings are fetched at runtime from Bunpro\'s public CDN; hosted/browser playback may use よむ\'s narrow public proxy.': 'Bunproの発音録音を、新しいオプトイン式音声ソースとして利用できるようになりました。新規・既存どちらの音声ソース一覧にも無効状態で追加されるため、利用者が有効にするまでBunproへの音声リクエストは行われません。録音は実行時にBunproの公開CDNから取得され、ホスト版やブラウザ再生ではよむの限定的な公開プロキシを使用する場合があります。',
