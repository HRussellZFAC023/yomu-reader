...
### SH-6: Deck management parity — v1 SHIPPED 0.6.97 (in-page JPDB deck selector on the Word tab: All vocabulary + user decks via the API, selection persists in new-tab state and rescopes the queue through the existing scheduled-only deck loader; mobile-safe 16px select). Per-deck progress SHIPPED 0.6.103 (deck entries show vocabulary count + known coverage from list-user-decks, jpdb Learn shape). Remaining: Jiten/Anki deck scoping (Jiten study-batch API has no deck param), deck creation; JPDB priority reorder stays page-only (no API endpoint)
