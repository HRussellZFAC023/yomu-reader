...
const HOSTED_DOCS_JA_COPY: Record<string, string> = {
    ...
    'Verified Electron\'s runtime executable before the Yomu Gaming smoke and release packaging, then downloaded the Electron runtime with `curl`, checked its SHA-256, and extracted it synchronously when GitHub Actions left a stale or skipped binary behind.': 'Yomu Gamingのスモークテストとリリースパッケージ化の前にElectronランタイム実行ファイルを検証し、GitHub Actionsに古い、またはスキップされたバイナリが残っている場合は`curl`でElectronランタイムをダウンロードし、SHA-256を確認してから同期的に展開するようにしました。',
