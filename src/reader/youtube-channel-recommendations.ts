...
export function youtubeChannelSubscribeUrl(channel: YouTubeChannelRecommendation): string {
    const url = new URL(youtubeChannelUrl(channel));
...
export function youtubeChannelRecommendationDescription(channel: YouTubeChannelRecommendation): string {
    ...
    return `${topics || 'Japanese'} videos around ${level}.${captionHint}`;
}
