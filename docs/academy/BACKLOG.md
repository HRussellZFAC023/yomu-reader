...
<a id="base-020"></a>
- [x] **BASE-020** Ship a four-line, five-binding native-band Aivis learning-voice slice with exact source/model/style/licence locks, archived canonical queries, deterministic Opus cache keys, request-owned cancellation and worker/browser fallback after initial and late media failure, objective acoustic/ASR QA, owner-approved output quality with `ownerLineByLineReviewed: false` and `humanReviewed: false`, and production-byte-parity route playback evidence. **Deps:** `BASE-002`. **Proof:** `C,R,T,Q,S,O`; archived four-model evidence is truthful but is not a future gate, human auditory acceptance remains open, and full voice production remains `AUD-001` to `AUD-005`.
...
<a id="aud-008"></a>
- [x] **AUD-008** Establish the per-slice voice production batch contract so a lesson, chapter, game or World beat can ship before global parity: immutable `voiceLineId`, speaker/intent/locale/native-band/source revision, required/excluded triage, exact model/style/licence lock, archived canonical query payloads and global/local style joins, deterministic render/cache identity, codec/acoustic checks, owner-approved quality with honest review fields, production-byte-parity route playback evidence and stale invalidation. **Deps:** `BASE-020`, `PED-001`. **Proof:** `C,R,T,Q,S,O` in `docs/academy/audio/learning-voice-production.json`, the generated v4 locks, and the v2 global voice manifest; this closes 4 voice units/5 bindings only, while human auditory acceptance and `AUD-001` to `AUD-005` remain open.
<a id="art-001"></a>
